...
extern void DrawImage(LPCTSTR szFileImage, CDC* pdc, CRect rect, int isOffset = 0);
...
#define LCD_HEIGHT  214
    ...
    void CMJPGStatic::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
    {
     ...
     void CMJPGStatic::OnSelchangeCombo1()
    {
        if (GetParent())
        {
            GetParent()->SendMessage(WM_COMMBOX_CLICKED, 0, 0);
        }
    }
...
#define MULITI_RECT_OFFSET 3
        ...
        else
        {
            ...
            else if (m_currentMJPGList->bgfilename_down != "")
            {
                ...
                if (m_currentMJPGList->bgfilename_down.Find(L".bmp") != -1 || m_currentMJPGList->bgfilename_down.Find(L".BMP") != -1)
                    ::DrawImage(ToFileDir(m_currentMJPGList->bgfilename_down), pdc, pUnit->m_Rect, 2);    //lxz  090225
                else
                {
                    CDC *pMemDC = new CDC();
                    CBitmap *pBmp = new CBitmap(); 
    ...
    {
        if (sFile.Find(_T("桌面.xml")) != -1) {
            //::MessageBox(NULL, _T("redraw desktop"), _T("redraw"), MB_TOPMOST);
        }
        if (m_currentMJPGList == NULL)
        {
            ::MessageBox(NULL, _T("Not draw surface"), _T("error"), MB_TOPMOST);
            return;
        }
        CreateMemDC();
