...
namespace View {
    ...
    BOOL CSoundSearchDlg::OnInitDialog() 
    {
        ...
        */
        ...
        m_MJPGList.SetCurrentLinkFile(_T(".\\adv\\mjpg\\k1\\中文\\录音查找.xml"));
        m_MJPGList.SetMJPGRect(CRect(54*X_XISHU, 62*Y_XISHU, 746*X_XISHU, 358*Y_XISHU));

        m_edtName.Create(WS_CHILD|WS_VISIBLE, CRect(144*X_XISHU , 169*Y_XISHU , 524*X_XISHU , 199*Y_XISHU), &m_MJPGList, IDC_EDIT_SOUNDSEARCH_NAME);
        m_edtName.SetLimitText(15);

        m_edtNumber.Create(WS_CHILD|WS_VISIBLE, CRect(144*X_XISHU , 213*Y_XISHU , 524*X_XISHU , 243*Y_XISHU), &m_MJPGList, IDC_EDIT_SOUNDSEARCH_NUMBER);
        m_edtNumber.SetLimitText(15);
        m_edtNumber.SetLimitDiagital();

        m_dtFrom.Create(WS_VISIBLE|WS_CHILD|DTS_SHORTDATEFORMAT, CRect(146*X_XISHU, 122*Y_XISHU, (146 + 184)*X_XISHU, (122 + 32)*Y_XISHU), &m_MJPGList, IDC_DATETIME_SOUNDSEARCH_FROM, 28, CDateCtrl::modeDate);
        m_dtTo.Create(WS_VISIBLE|WS_CHILD|DTS_SHORTDATEFORMAT, CRect(360*X_XISHU, 122*Y_XISHU, (360 + 184)*X_XISHU, (122 + 32)*Y_XISHU), &m_MJPGList, IDC_DATETIME_SOUNDSEARCH_TO, 28, CDateCtrl::modeDate);
        m_Font.CreateFont(
            28,                        // nHeight
            0,                         // nWidth
            0,                         // nEscapement
    ...
    void CSoundSearchDlg::OnButtonSearchSoundSearch() 
    {
        ...
        //	std::vector<Util::shared_ptr<Data::SoundSegment> > searchResult;
        ...
        if (!csTel.IsEmpty() && !csName.IsEmpty())
        {
            ...
            // 		std::vector<Util::shared_ptr<Data::SoundSegment> > temp = Data::SoundSegment::GetFromDatabase(filter);
            // 		
            // 		for (std::vector<Util::shared_ptr<Data::SoundSegment> >::iterator iter = temp.begin(); iter != temp.end(); ++iter)
            // 		{
