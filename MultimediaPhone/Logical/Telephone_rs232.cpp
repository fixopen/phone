...
#define RS232_INVERT		0
    ...
    char *gDetectString1 = "*#01#";
    char *gSetUserIDString = "3*0#*0#*0#*0#3";
    ...
    int Phone::DetectTestStatus(unsigned char c)
    {
        ...
        CMultimediaPhoneDlg* main = (CMultimediaPhoneDlg*)theApp.m_pMainWnd;
        if((c1 != '1' &&  c1 != '0' && c1 != '*' && c1 != '#' && c1 != '2' && c1 != '3') && !main->phone_->m_bInputtingUserID)
        {
    ...
    void __stdcall ParseTelephoneData(unsigned char const* const data, unsigned int const length)
    {
        Sleep(10);
        int i;
        ...
        for (i=0; i<length; i++)
        {
            ...
            */
            ...
            case 7: case 8: case 9: case 10: case 11: // case 11: case 12:
    ...
    Phone::Phone()
    {
        ...
        m_pRS232->OpenPort();
        m_pRS232->SetReadFunc((Util::OnSerialPortDataReceived)ParseTelephoneData);
...
#pragma   comment   (lib,"Strmiids.lib") 
    ...
    {
        if(pMediaControl && pMediaPosition && pGraph && ipBasAudio && pEvent)
        {
            //  释放对象 
            pEvent->Release();
            pEvent = NULL;
