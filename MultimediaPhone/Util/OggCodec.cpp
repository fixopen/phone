...
namespace Util {
    ...
    unsigned long OggCodec::Encode() {
        ...
        comment_init(&comments_enc, &comments_length_enc, vendor_string);
        comment_add(&comments_enc, &comments_length_enc, "title=", "yidu"); 
    ...
    bool OggCodec::DecodeSeek(float percent)
    {
        if (!bOggDecStop && (fin_dec != NULL) && (file_size > 0))
        {
            int pos = file_size * percent;
            pos += 158;//158 file head
            EnterCriticalSection(&csDecodeFile);
            fseek(fin_dec, pos, SEEK_SET);
            m_bDecodeSeek = true;
            LeaveCriticalSection(&csDecodeFile);
            return true;
        }
    ...
    void OggCodec::EncodeFile(std::string encodeFileName)
    {
        sEncFileName = encodeFileName;
    }

    void OggCodec::DecodeFile(std::string decodeFileName)
    {
    ...
    void OggCodec::comment_init(char **comments, int* length, char *vendor_string)
    {
    ...
    void OggCodec::comment_add(char **comments, int* length, char *tag, char *val)
    {
        char* p=*comments;
