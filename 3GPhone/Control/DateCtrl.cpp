...
namespace Control {
    ...
    BOOL CDateCtrl::Create(DWORD dwStyle, const RECT& rect_1, CWnd* pParentWnd, UINT nID, UINT uFontHeight, em_mode mode) 
    {
        ...
        //    if (!m_btnUp.Create(_T(""), IDB_BITMAP_LEFTARROW, IDB_BITMAP_LEFTARROWD, WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON, rcButton, this, ID_BUTTON_UP))
        //        return FALSE;
    ...
    void CDateCtrl::OnLButtonDown(UINT nFlags, CPoint point)
    {
        ...
        CSize szY = pDC->GetTextExtent(_T("XXXX"));
        CSize szM = pDC->GetTextExtent(_T("XX"));
        CSize szF = pDC->GetTextExtent(_T("-"));
