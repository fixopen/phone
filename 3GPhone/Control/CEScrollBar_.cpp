...
namespace Control {
    ...
    void CCEScrollBar_::OnLButtonUp(UINT nFlags, CPoint point) 
    {
        ...
        if(m_nPressed == 5)
        {
            ...
            if(m_Owner)
            {
                m_Owner->SendMessage(WM_VSCROLL, MAKELONG(SB_THUMBPOSITION, 0), nPos+1);
            }
            m_nPos = nPos;
            SetPos(m_nPos);
            /*CPoint pt1;
        ...
        if(m_RectUpArrow.PtInRect(point))
        {
            ...
            // 		if(m_nPos > 0)
            {
                ...
                if (m_nPos < 0)//mhq
                    m_nPos = 0;
                if(m_Owner)
                    m_Owner->SendMessage(WM_VSCROLL, SB_LINEUP, m_nPos+1 ) ;
                SetPos(m_nPos);
            }
        }
        ...
        else if(m_RectDownArrow.PtInRect(point))	//下翻一行
        {
            ...
            //		if(m_nPos < (m_nMax - m_nMin - m_nPage))  //mhq
            {
                ...
                if(m_Owner)
                    m_Owner->SendMessage(WM_VSCROLL, SB_LINEDOWN, m_nPos+1 ) ;
                SetPos(m_nPos);
            }
        }
        else if(m_RectUp.PtInRect(point)) //上翻一页
        {
            ...
            if(m_nPos > 0)
            {
                ...
                if(m_Owner)
                {
                    m_Owner->SendMessage(WM_VSCROLL, MAKELONG(SB_PAGEUP,0), m_nPos+1);
                }
                SetPos(m_nPos);
                /*
                CRect rt;
                ...
                if(m_Owner)
                {
                m_Owner->SendMessage(WM_VSCROLL, MAKELONG(SB_PAGEUP, 0), nPos);
                }
        ...
        else if(m_RectDown.PtInRect(point)) //下翻一页
        {
            ...
            if(m_nPos < (m_nMax - m_nMin - m_nPage))  //mhq
            {
                ...
                if(m_Owner)
                {
                    m_Owner->SendMessage(WM_VSCROLL, MAKELONG(SB_PAGEDOWN,0),m_nPos+1);
                }
                SetPos(m_nPos);
                ...
                if(m_Owner)
                {
                m_Owner->SendMessage(WM_VSCROLL, MAKELONG(SB_PAGEDOWN, 0), nPos);
                }
                m_nPos = nPos*(m_nMax - m_nMin)/100;
                SetPos(m_nPos);
                */
        ...
        if(m_nPressed == 5)	//Thick
        {
            ...
            if(abs(point.y - oldpoint.y) >= 10 )
            {
                ...
                */
                ...
                // 			if(m_Owner)
                // 			{
                // 				m_Owner->SendMessage(WM_VSCROLL, MAKELONG(SB_THUMBPOSITION, 0), nPos);
                // 			}
                // 			SetPos(m_nPos);
            }
        }
