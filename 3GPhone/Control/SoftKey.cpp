...
#include "stdafx.h"
#include "SoftKey.h"
#include "../Util/LanguageResource.h"

namespace Control {
    int const IDC_REG_TIMER = 0x1000;

    int const TOGGLEBMP_WIDTH = 90;
    int const MARGIN_PIXEL = 2;
    int const MARGINDOWN_PIXEL = 4;
    int const ICON_PIXEL = 16;
    ...
    void InitWriteLineBuff()
    {
        if(m_pWriteLineBuff == NULL)
        {
            ...
            if(m_pWriteLineBuff)
            {
                ...
                if(hWriteThread)
                {
                    int ret = 0; //HWRE_LoadDict(&g_writedic);
                    if(ret != 0)
                    {
                        delete m_pWriteLineBuff;
                        m_pWriteLineBuff = NULL;			
