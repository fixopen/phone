...
namespace Util {
    std::wstring Contact::Data::Contact::tableName_ = L"contact";
    int Contact::Data::Contact::count_ = 0;
    int Contact::Data::Contact::offset_ = 0;
}
...
namespace Contact {
    namespace Data {
        Util::shared_ptr<ContactGroup> Contact::group() {
            if (!groupFinded_) {
                std::vector<Util::shared_ptr<ContactGroup> > t = ContactGroup::GetFromDatabase(L"id = " + Util::StringOp::FromInt(groupId_));
                //assert(t.size() == 1);
                if (t.size() == 1) {
                    group_ = t[0];
                }
        ...
        void Contact::modifyFieldByDB_(int argc, char** argv, char** columnName, Util::shared_ptr<Contact> item) {
            ...
            item->mobilesTelephone_[0] = TelephoneNumber(argv[GetIndexByName(argc, columnName, "mobilesTelephone")], true);
            item->mobilesTelephone_[1] = TelephoneNumber(argv[GetIndexByName(argc, columnName, "mobilesTelephone2")], true);
            item->worksTelephone_[0] = TelephoneNumber(argv[GetIndexByName(argc, columnName, "worksTelephone")], true);
            item->worksTelephone_[1] = TelephoneNumber(argv[GetIndexByName(argc, columnName, "worksTelephone2")], true);
            item->internalTelephone_ = TelephoneNumber(argv[GetIndexByName(argc, columnName, "internalTelephone")], true);
            item->fax_ = TelephoneNumber(argv[GetIndexByName(argc, columnName, "fax")], true);
            item->familyTelephone_ = TelephoneNumber(argv[GetIndexByName(argc, columnName, "familyTelephone")], true);
