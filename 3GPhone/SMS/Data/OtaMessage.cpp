...
namespace Util {
    std::wstring SMS::Data::OtaMessage::tableName_ = L"otaMessage";
    int SMS::Data::OtaMessage::count_ = 0;
    int SMS::Data::OtaMessage::offset_ = 0;
}

namespace SMS {
    namespace Data {
        std::vector<Util::shared_ptr<OtaMessage> > const OtaMessage::Select(std::wstring const& filter) {
        ...
        void OtaMessage::Insert() {
            ...
            cmd += tableName_;
            cmd += L" (smscAddressType, smscAddress, isSetReplyPath, hasHeaderInfo, requestReport, validityPeriodFormat, rejectCopy, type, reference, remoteAddressType, remoteAddress, uplevelProtocol, encoding, validityPeriod, timestamp, state, instruction, url ) VALUES ( " ; 
            cmd += Util::StringOp::FromInt(smsc.addressType.typeOfNumber);
            cmd += L", '";
            cmd += Util::StringOp::FromUTF8(smsc.address);
            cmd += L"', ";
        ...
        void OtaMessage::Remove(std::wstring const& filter) {
        ...
        void OtaMessage::modifyFieldByDB_(int argc, char** argv, char** columnName, Util::shared_ptr<OtaMessage> item) {
