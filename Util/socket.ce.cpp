...
std::string UDP::GetLocalIP(void) {
    ...
    if (gethostname(buffer, 256) == 0) {
        ...
        memset(buffer, 0, 256);
        sprintf_s(buffer, 256, "%d.%d.%d.%d", LocalAddress->h_addr_list[0][0] & 0xFF, LocalAddress->h_addr_list[0][1] & 0xFF, LocalAddress->h_addr_list[0][2] & 0xFF, LocalAddress->h_addr_list[0][3] & 0xFF);
            ...
            if (FD_ISSET(socket->handle_, &fdRead)) {
                ...
                if (recvLen == SOCKET_ERROR) {
                    ...
                    break;
                      ...
                      //  printf("%x ", recvBuf[i] & 0xFF);
                      //printf("\n");
                      socket->onReceived(recvBuf, recvLen); //, (SOCKADDR*)&tmpAddr
                      //delete[] recvBuf;
                      //recvBuf = NULL;
                }
            }
