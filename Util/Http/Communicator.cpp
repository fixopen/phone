...
namespace Util {
    namespace Http {
        ...
        ResponseMessage const Communicator::Get(RequestMessage const& message) const {
            ...
            std::vector<std::string> parts = Util::Text::StringOp::Split(received, "\r\n\r\n");
            std::vector<std::string> headers = Util::Text::StringOp::Split(parts[0], "\r\n");
            ...
            if (pos != std::string::npos) {
                ...
                if (pos != std::string::npos) {
                    result.state = static_cast<ResponseMessage::State>(Util::Text::StringOp::ToInt(headers[0].substr(offset, pos - offset)));
                    offset = pos + 1;
            ...
            for (size_t i = 1; i < headers.size(); ++i) {
                ...
                std::string name = Util::Text::StringOp::Trim(headers[i].substr(0, del));
                std::string value = Util::Text::StringOp::Trim(headers[i].substr(del + 1));
                result.HeaderFields.insert(std::make_pair(name, value));
                if (name == "Content-Length") {
                    contentLength = Util::Text::StringOp::ToInt(value);
                }
            }
            result.Body = parts[1];
            if (contentLength) {
