...
// 			second = now.GetSecond();
	...
	std::string StringOp::ToUTF8(const std::string& str)
	{
		CString strBody = Util::StringOp::ToCString(str);
		int length = strBody.GetLength() * 3 + 1;
		char* out = new char[length];
		memset(out, 0, length);
		int len = Util::StringOp::UnicodeToUTF_8(out, (LPTSTR)(LPCTSTR)strBody, length, strBody.GetLength());
		std::string utf8(out, len);
		delete [] out;
		return utf8;
	}
