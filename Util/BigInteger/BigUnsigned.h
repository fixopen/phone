...
protected:
	...
	void initFromPrimitive(unsigned long x) {
		if (x == 0)
	...
	void initFromPrimitive(unsigned int x) {
		if (x == 0)
			; // NumberlikeArray already initialized us to zero.
		else {
			// Create a single block.  blk is NULL; no need to delete it.
	...
	void initFromPrimitive(unsigned short x) {
		if (x == 0)
			; // NumberlikeArray already initialized us to zero.
		else {
			// Create a single block.  blk is NULL; no need to delete it.
	...
	void initFromPrimitive( short x) {
		if (x == 0)
			; // NumberlikeArray already initialized us to zero.
		else {
			// Create a single block.  blk is NULL; no need to delete it.
	...
	void initFromSignedPrimitive(unsigned long x) {
		if (x < 0)
	...
	void initFromSignedPrimitive(unsigned int x) {
		if (x < 0)
			throw "BigUnsigned constructor: "
				"Cannot construct a BigUnsigned from a negative number";
		else
	...
	void initFromSignedPrimitive(unsigned short x) {
		if (x < 0)
			throw "BigUnsigned constructor: "
				"Cannot construct a BigUnsigned from a negative number";
		else
	...
	void initFromSignedPrimitive( long x) {
		if (x < 0)
			throw "BigUnsigned constructor: "
				"Cannot construct a BigUnsigned from a negative number";
		else
	...
	void initFromSignedPrimitive( int x) {
		if (x < 0)
			throw "BigUnsigned constructor: "
				"Cannot construct a BigUnsigned from a negative number";
		else
	...
	void initFromSignedPrimitive( short x) {
		if (x < 0)
			throw "BigUnsigned constructor: "
				"Cannot construct a BigUnsigned from a negative number";
		else
...
protected:
	...
	unsigned long convertToSignedPrimitive(unsigned long r) const {
		unsigned long x = convertToPrimitive(r);
		if (x >= 0)
			return x;
		else
	...
	unsigned int convertToSignedPrimitive(unsigned int r) const {
		//unsigned int r = 0;
		unsigned int x = convertToPrimitive(r);
		if (x >= 0)
			return x;
		else
			throw "BigUnsigned::to(Primitive): "
	...
	unsigned short convertToSignedPrimitive(unsigned short r) const {
		//unsigned short r = 0;
		unsigned short x = convertToPrimitive(r);
		if (x >= 0)
			return x;
		else
			throw "BigUnsigned::to(Primitive): "
	 ...
	 long convertToSignedPrimitive(long r) const {
		 long x = convertToPrimitive(0l);
		if (x >= 0)
			return x;
		else
			throw "BigUnsigned::to(Primitive): "
	 ...
	 int convertToSignedPrimitive(int r) const {
		 int x = convertToPrimitive(r);
		if (x >= 0)
			return x;
		else
			throw "BigUnsigned::to(Primitive): "
	 ...
	 short convertToSignedPrimitive(short r) const {
		 //short r = 0;
		 short x = convertToPrimitive(r);
		if (x >= 0)
			return x;
		else
			throw "BigUnsigned::to(Primitive): "
	...
	unsigned long convertToPrimitive      (unsigned long r) const {
		if (len == 0)
			// The number is zero; return zero.
			return 0;
		else if (len == 1) {
			// The single block might fit in an X.  Try the conversion.
			unsigned long x = unsigned long(blk[0]);
	...
	unsigned int convertToPrimitive      (unsigned int r) const {
		if (len == 0)
			// The number is zero; return zero.
			return 0;
		else if (len == 1) {
			// The single block might fit in an X.  Try the conversion.
			unsigned int x = unsigned int(blk[0]);
			// Make sure the result accurately represents the block.
			if (Blk(x) == blk[0])
				// Successful conversion.
				return x;
	...
	unsigned short convertToPrimitive      (unsigned short r) const {
		if (len == 0)
			// The number is zero; return zero.
			return 0;
		else if (len == 1) {
			// The single block might fit in an X.  Try the conversion.
			unsigned short x = unsigned short(blk[0]);
			// Make sure the result accurately represents the block.
			if (Blk(x) == blk[0])
				// Successful conversion.
				return x;
	 ...
	 long convertToPrimitive      ( long r) const {
		if (len == 0)
			// The number is zero; return zero.
			return 0;
		else if (len == 1) {
	 ...
	 int convertToPrimitive      ( int r) const {
		if (len == 0)
			// The number is zero; return zero.
			return 0;
		else if (len == 1) {
	 ...
	 short convertToPrimitive      ( short r) const {
		if (len == 0)
			// The number is zero; return zero.
			return 0;
		else if (len == 1) {
