...
namespace Util
{
    ...
    : port(80) {
        ...
        if (stopPos != std::wstring::npos) {
            ...
            if (portStartPos != std::wstring::npos) {
                //find port
    ...
    void HTTPTransport::SetAPN(DIAL_APN apn, char* proxy) {
        ...
        mbstowcs_s(&convertCount, apn_proxyIP, 32, proxy, strlen(proxy) + 1);
    }

    DWORD HTTPTransport::Get(std::wstring const & uriV, std::wstring const & agentName) {
        Dprintf("Get HTTPTransport 0\r\n");
        DWORD error = 0;
        HINTERNET HTTPOpen = ::InternetOpen(agentName.c_str(), INTERNET_OPEN_TYPE_PROXY /*INTERNET_OPEN_TYPE_PRECONFIG*/, // proxy option
            apn_proxyIP, // proxy
            NULL, // proxy bypass
            0); // flags
        ...
        if (!HTTPOpen) {
            ...
            HINTERNET HTTPConnection = ::InternetConnect(HTTPOpen, // internet opened handle
                uri.host.c_str(), uri.port, uri.username.c_str(), uri.password.c_str(), INTERNET_SERVICE_HTTP, // service type
            ...
            if (!HTTPConnection) {
                ...
                if (::InternetAttemptConnect(NULL) != ERROR_SUCCESS) {
                    ...
                    wchar_t data[256] = {
                      ...
                      wcscpy_s(data, 256, HttpAcceptType.c_str());
                    ...
                    LPCTSTR ppszAcceptTypes[2];
                      ...
                      */
                      HTTPRequest = ::HttpOpenRequest(HTTPConnection, HttpVerbs[vGet].c_str(), // HTTP Verb
                        uriV.c_str(), // Object Name
                        HTTP_VERSION, // Version
                        NULL, // Reference
                        /*&acceptType*/ppszAcceptTypes, // Accept Type
                        INTERNET_FLAG_RELOAD | INTERNET_FLAG_DONT_CACHE | INTERNET_FLAG_KEEP_CONNECTION, 0); // context call-back point
            ...
            if (!HTTPConnection) {
                ...
                if (::InternetAttemptConnect(NULL) != ERROR_SUCCESS) {
                    ...
                    LPCTSTR ppszAcceptTypes[2];
                      ...
                      HTTPRequest = ::HttpOpenRequest(HTTPConnection, HttpVerbs[vPost].c_str(), // HTTP Verb
                        uriV.c_str(), // Object Name
                        HTTP_VERSION, // Version
                        NULL, // Reference
                        ...
                        if (!::HttpAddRequestHeaders(HTTPRequest, HttpAccept.c_str(), HttpAccept.length(), HTTP_ADDREQ_FLAG_REPLACE)) {
                            ...
                            }
                            {
                                ...
                                if (!::HttpSendRequest(HTTPRequest, // handle by returned HttpOpenRequest
                                    NULL, // additional HTTP header
                                    0, // additional HTTP header length
                                    pPostData, nPostDataLenth/*(void *)message.body.c_str(),*/ // additional data in HTTP Post or HTTP Put
                                    /*message.body.length()*/)) {
                                    // additional data length
                                    error = ::GetLastError();
                                } else {
    ...
    DWORD HTTPTransport::Get_(std::wstring const & uriV) {
        ...
        if (pNewSocket.Connect(sIp, port)) {
            ...
            size_t convertCount = 0;
            wcstombs_s(&convertCount, url_, 256, uriV.c_str(), uriV.length());
            sprintf_s(BUF_, 1024, "GET %s HTTP/1.1\r\nAccept:*/*\r\nAccept-Language:zh-cn\r\nUser-Agent:3GPHONE(0.9/;p)\r\nConnection:Keep-Alive\r\n\r\n", url_);
    ...
    DWORD HTTPTransport::Post_(std::wstring const & uriV) {
        ...
        if (pNewSocket.Connect(sIp, port)) {
            ...
            sprintf_s(BUF_, 1024, "%s%s%s%d%s", pBuf[0], url_, pBuf[1], nPostDataLenth, pBuf[2]);
            strcat_s(BUF_, 1024, pPostData);
            int ret = pNewSocket.Send(BUF_, strlen(BUF_));
            if (ret != strlen(BUF_)) {
                error = GetLastError();
            }
