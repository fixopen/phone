...
int SMSWarp::gsmEncodePdu_(const SM_PARAM* pSrc, SMS_BUFF* pDst_) {
    ...
    for (int i = 0; i < nTotal; i++) {
        ...
        if (nTotal > 1) {
            ...
            sprintf_s(head_, 25, "%02x%02x%02x%02x%02x%02x", head[0] & 0xFF, head[1] & 0xFF, head[2] & 0xFF, head[3] & 0xFF, head[4] & 0xFF, head[5] & 0xFF);
            strcpy_s(&pDst[nDstLength], 512 - nDstLength, head_);
            nDstLength += 12;
            nDstLength += gsmBytes2String(buf + 4, &pDst[nDstLength], nLength - 4);
            strcpy_s(pDst_[nCount].buff, 1024, pDst);
...
int SMSWarp::gsmSendMessage(SM_PARAM* pSrc, SM_BUFF* pBuff) {
    ...
    */
    ...
    for (int i = 0; i < count; i++) {
        ...
        strcat_s(pdu[i].buff, 1024, "\x01a");       // 以Ctrl-Z结束    
...
int SMSWarp::gsmParseMessageList(SM_PARAM* pMsg, SM_BUFF* pBuff) {
    ...
    while ((ptr = strstr(ptr, "+CMGL:")) != NULL) {
        ptr += 6;       // 跳过"+CMGL:", 定位到序号    
        sscanf_s(ptr, "%d", &pMsg->index); // 读取序号    
        //      TRACE("  index=%d\n",pMsg->index);    
...
void SMSWarp::Send(std::string number, std::string content) {
    ...
    if (sca.substr(0, 2) != "86") {
        sca = "86" + sca;
    }   

    // 填充短消息结构    
    strcpy_s(SmParam.SCA, 32, sca.c_str());   
    strcpy_s(SmParam.TP_UD, 1024, content.c_str());   
    SmParam.TP_PID = 0;   
    ...
    do {
        ...
        if (number_ != "") {
            if (number_.substr(0, 2) != "10") {
                ...
                if (number_.substr(0, 2) != "86") {
                    number_ = "86" + number_;
                }
            }
            strcpy_s(SmParam.TPA, 32, number_.c_str());   
