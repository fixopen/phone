...
namespace Util
{
	...
	{
		// 注意 WCHAR高低字的顺序,低字节在前-
        enum Hex
		{ 
			Hex80 = 0x80, Hex800 = 0x800, Hex10000 = 0x10000, Hex200000 = 0x200000
	...
	std::string const StringOp::ToUTF8(std::string const& content) 
	{
		...
		for (/*std::*/size_t i = 0; i < length; ++i) 
		{
			...
			{ //3byte
				...
				chr = ((char)(unicode.GetAt(i) & 0x3F)) | 0x80;
				result += chr;
			} else if (unicode.GetAt(i) < Hex200000) { //4byte
				//
			} else if (unicode.GetAt(i) < Hex4000000) { //5byte
				//
			} else if (unicode.GetAt(i) < Hex80000000) { //6byte
				//
			}
		}
		return result;
	...
	void StringOp::utf8ToUnicode(const std::string& src, std::wstring& result)
	{
	...
	void StringOp::unicodeToUTF8(const std::wstring &src, std::string& result)
	{
		int n = WideCharToMultiByte( CP_UTF8, 0, src.c_str(), -1, 0, 0, 0, 0 );  
		result.resize(n);  
		::WideCharToMultiByte( CP_UTF8, 0, src.c_str(), -1, (char*)result.c_str(), result.length(), 0, 0 );
	}
	void StringOp::gb2312ToUnicode(const std::string& src, std::wstring& result)
	{
		int n = MultiByteToWideChar( CP_ACP, 0, src.c_str(), -1, NULL, 0 );  
		result.resize(n);  
		::MultiByteToWideChar( CP_ACP, 0, src.c_str(), -1, (LPWSTR)result.c_str(), result.length());  
	}
	void StringOp::unicodeToGB2312(const std::wstring& wstr , std::string& result)
	{
		int n = WideCharToMultiByte( CP_ACP, 0, wstr.c_str(), -1, 0, 0, 0, 0 );  
		result.resize(n);  
		::WideCharToMultiByte(CP_ACP, 0, wstr.c_str(), -1, (char*)result.c_str(), n, 0, 0 );
	}
	void StringOp::gb2312TOutf8(const std::string &src,std::string &result)
	{
		std::wstring temp;
		gb2312ToUnicode(src,temp);
		unicodeToUTF8(temp,result);
	}
	void StringOp::utf8Togb2312(const std::string& src, std::string& result)
	{
		std::wstring temp;
		utf8ToUnicode(src,temp);
		unicodeToGB2312(temp,result);
	}
}
