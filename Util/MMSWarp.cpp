...
int MMSWarp::EncodeSubmitRequest(const MMS_SubmitRequest* pSrc, char* pDst) {
    int length = 0;
    pDst[length++] = '\x8C'; //X-Mms-Message-Type被编码成8Ch，m-send-req被编码成80h
    pDst[length++] = '\x80';
    pDst[length++] = '\x98'; //98h代表Transaction-ID
    memcpy(pDst + length, pSrc->TransactionID.c_str(), pSrc->TransactionID.size());
    length += pSrc->TransactionID.size();
    pDst[length++] = '\x00';
    pDst[length++] = '\x8D'; //8Dh是MMS-Version，而90h相对应的值为1.0
    pDst[length++] = '\x90';
    pDst[length++] = '\x89'; //89h表示源（From）
    ...
    sprintf_s(psTime, 32, "%x", seconds);
    pDst[length++] = '\x85'; //Date
    ...
    pDst[length++] = '\x8F';//
    pDst[length++] = '\x80';
    pDst[length++] = '\x84';//84h表示Content-Type
    pDst[length++] = '\x1C';
    pDst[length++] = '\xB3';//B3h表示application/vnd.wap.multipart.related  A3—表示:application/vnd.wap.multipart.mixed
    pDst[length++] = '\x8A';//8Ah表示起始参数
    memcpy(pDst + length, "<start>", strlen("<start>"));
    length += strlen("<start>");
    pDst[length++] = '\x00';
    pDst[length++] = '\x89';//89h表示类型参数，其值是以纯文本—“application/smil”格式跟 随其后
    ...
    _itoa_s(pSrc->layout.root_layout.width, width, 8, 10);
    smil += width;
    smil += "\" ";
    smil += "height=\"";
    char height[8] = {
    ...
    _itoa_s(pSrc->layout.root_layout.height, height, 8, 10);
    smil += height;
    smil += "\"/>\r\n";
    ...
    _itoa_s(region.width, width, 8, 10);
    smil += width;
    smil += "\" ";
    smil += "height=\"";
    memset(height, 0, 8);
    _itoa_s(region.height, height, 8, 10);
    smil += height;
    smil += "\" ";
    smil += "left=\"";
    char left[8] = {
    ...
    for (/*vector<MMS_PAR>::iterator */iter = pars.begin(); iter != pars.end(); ++iter) {
        ...
        _itoa_s((*iter).duration, duration, 8, 10);
        smil += duration;
        smil += "s\">\r\n";
        ...
        for (vector<MMS_SRC>::iterator i = (*iter).srcs.begin(); i != (*iter).srcs.end(); ++i) {
            ...
            sprintf_s(namebuf, 128, "file%d", index++);
            name = namebuf + suffix;
        ...
        if (flag == '\x8C') {
            ...
            if (token == '\x81') {
                ...
                CTimeSpan ts = expiry - start;
                  n = (unsigned int)ts.GetTotalSeconds();
                  length += len;
            }
              pDst->TimeOfExpiry = n;
        } else if (flag == '\x89') {
...
Data::MMSData* MMSWarp::DecodeMessage(const unsigned char* pSrc, int srcLen) {
    ...
    sprintf_s(buf, 128, "/NandFlash/resource/3G/MMS/%d%02d%02d%02d%02d%02d/", t.GetYear(), t.GetMonth(), t.GetDay(), t.GetHour(), t.GetMinute(), t.GetSecond());
    msg->SavePath = buf;
    CreateDirectory(_3g_mms_dir, NULL);
                                ...
                                while (true) {
                                    if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                                        ...
                                        if (mms_src.type == stImge) {
                                            if ((suffix == "jpg") || (suffix == "gif") || (suffix == "bmp")) {
                                                name = path + file;
                                                break;
                                            }
                                            ...
                                            if ((suffix == "amr") || (suffix == "mid") || (suffix == "wav")) {
                                                name = path + file;
                                                break;
                                            }
                                        }
...
void MMSWarp::EncodeSmil(vector<MMS_PAR> vpars, MMS_LAYOUT layout, std::string path) {
    ...
    _itoa_s(layout.root_layout.width, width, 8, 10);
    smil += width;
    smil += "\" ";
    smil += "height=\"";
    char height[8] = {
    ...
    _itoa_s(layout.root_layout.height, height, 8, 10);
    smil += height;
    smil += "\"/>\r\n";
    smil += "      <region id=\"Image\" ";
    smil += "width=\"";
    ...
    FILE* f = 0;
    fopen_s(&f, std::string(path + "start.smil").c_str(), "wb");
...
void MMSWarp::AddPar(MMS_SubmitRequest* req, string imageFile, string textFile, string audioFile, unsigned int dur) {
    ...
    if (!audioFile.empty()) {
        MMS_SRC audio;
        audio.type = MMS::MMSWarp::stAudio;
