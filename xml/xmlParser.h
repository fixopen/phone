...
namespace Util
{
    ...
    private:
        ...
        static void LoadXMLFile(IXMLDOMDocument* parser, std::string filename) {
            ...
            mbstowcs_s(&convertCount, temp, fileNameLength, filename.c_str(), fileNameLength);
            //@@
        ...
        static std::string GetText(IXMLDOMNode* item) {
            ...
            memset(temp, 0, 2 * length + 1);
            //@@
            size_t convertCount = 0;
            wcstombs_s(&convertCount, temp, 2 * length + 1, nodeContent.m_str, 2 * length);
            //@@
        ...
        static std::string GetNodeName(IXMLDOMNode* item) {
            ...
            memset(temp, 0, 2 * length + 1);
            size_t convertCount = 0;
            wcstombs_s(&convertCount, temp, 2 * length + 1, nodeName.m_str, 2 * length);
    ...
    public:
        ...
        static char* FindElementsContent(char* sXmlFileBuff, char* sName) {
            ...
            sprintf_s(sStart, 64, "<%s>", sName);
            char sEnd[64] = {
                0
            };
            sprintf_s(sEnd, 64, "</%s>", sName);
