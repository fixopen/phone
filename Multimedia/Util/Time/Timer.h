#ifndef __UTIL_TIME_TIMER_H__
#define __UTIL_TIME_TIMER_H__
...
namespace Util {
    namespace Time {
        ...
        public:
            ...
            enum Policy {
                tpNull,
                tpAutoCancel, /*超时后自动销毁自身，缺省*/
                tpAutoRestart, /*超时后自动重起，也就是所谓的周期性重复时钟*/
                tpStiff, /*超时候变成僵尸*/
                tpOther
            };
        public:
            Timer() //constructor the null timer
                : id_(0)
                , type_(Other)
                //, registerTick_(0)
                , timeout_(0)
