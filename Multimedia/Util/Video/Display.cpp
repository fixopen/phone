...
namespace Util {
    namespace Video {
        ...
        bool Display::SetParameters(HWND const videoWindow, size_t const videoWidth, size_t const videoHeight, bool const isLocal) {
            ...
            ::GetWindowRect(ownerWindow_, &bounds);
            winWidth_ = bounds.right - bounds.left;
            winHeight_ = bounds.bottom - bounds.top;

            if (false/*!dxRender_->Init(videoWindow, videoWidth, videoHeight, SURFACE_TYPE_RGB565)*/) {
                DWORD error = GetLastError();
                //TRACE1("mfc_render_init() Failed with: %i\n", error);
                return false;
            }

            render_ = 0; //dxRender_->CreateSurface(SURFACE_TYPE_RGB565, 0, 0, videoWidth, videoHeight, videoWidth, videoHeight);
        ...
        bool Display::Show(Image::ImageFormat const imageFormat, unsigned int const videoWidth, unsigned int const videoHeight, unsigned char const* const videoData) {
            bool result = true;
        ...
        void Display::convertYUVtoRGB_(unsigned char const* const yuvData, Image::ImageFormat const yuvFormat, size_t const width, size_t const height, unsigned char* const rgb888Data) {
