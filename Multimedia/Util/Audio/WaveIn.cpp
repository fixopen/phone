...
namespace Util {
    namespace Audio {
        WaveIn* const WaveIn::Instance() {
            static WaveIn* result = 0;
            ...
            void CALLBACK waveCallback(HWAVE hWave, UINT msg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2) {
                ...
                switch (msg) {
                    case WIM_OPEN:
                        break;
                    case WIM_DATA:
                        WaveIn::Instance()->Encoder()->Encode(reinterpret_cast<spx_int16_t*>(p->lpData), p->dwBytesRecorded / sizeof(short));
                        break;
                    case WIM_CLOSE:
                        break;
                    default:
        ...
        std::string const WaveIn::SpitData() {
            std::string result;
            return result;
        }

        bool const WaveIn::Start(std::wstring const& filename, std::wstring const& comment, bool const isBusyToneDetect) {
            bool result = false;
