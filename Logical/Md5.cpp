...
bool MD5_File (LPCTSTR filename, unsigned char md5[16])
{
    HANDLE file;
    MD5Context context;
	...
	file = CreateFile (filename, // Pointer to the name of the port
    ...
    else 
    {
		...
		while ( ReadFile(file,buffer,1024,&dwSize,NULL) )
		{
			if (dwSize == 0)
			{
				result = true;
