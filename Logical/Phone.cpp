...
namespace Logical//scan html text fire follow event
{
    ...
    , ringCount_(0)
    {
        ...
        fsm_.registerRule(tsIdle, teHandsfree, tsHandsfree, handsfree);
        ...
        fsm_.registerRule(tsHandsfree, teHandsfree, tsIdle, hangupNotRecord);
        fsm_.registerRule(tsHandsfree, teKey, tsDialKey, keyAndStartTimeout);
        fsm_.registerRule(tsHandsfree, teTimeout, tsError, dialTimeout);

        fsm_.registerRule(tsDialKey, teConnect, tsConnected, connect);
        fsm_.registerRule(tsDialKey, teHangup, tsIdle);
        fsm_.registerRule(tsDialKey, teHandsfree, tsIdle);
        fsm_.registerRule(tsDialKey, teKey, tsDialKey, keyAndStartTimeout);

        fsm_.registerRule(tsRing, tePickup, tsConnected, connect);
        fsm_.registerRule(tsRing, teRemoteHangup, tsIdle, remoteHangup);
        ...
        fsm_.registerRule(tsConnected, teError, tsError, communicationError);
        ...
        fsm_.registerRule(tsRecording, teError, tsConnected, stopRecord);
        fsm_.registerRule(tsRecording, teHangup, tsIdle, stopRecord);
        fsm_.registerRule(tsRecording, teRemoteHangup, tsIdle, stopRecord);
        ...
        fsm_.registerRule(tsPlaying, teHangup, tsIdle, stopPlay);
        fsm_.registerRule(tsPlaying, teRemoteHangup, tsIdle, stopPlay);
    ...
    int const Phone::run()
    {
        ...
        while (!quit_)
        {
            ...
            unsigned char* parsed = (unsigned char*)malloc(count + 1);
            memset(parsed, 0, count + 1);
    ...
    /*
    void Phone::ConvertASCIItoDTMF_(unsigned char* const dst, unsigned char const* const src, unsigned int const length)
    {
    ...
    */
    ...
    /*
    void Phone::ConvertASCIItoFSK_(unsigned char* const dst, unsigned char const* const src, unsigned int const length)
    {
    }
    */
