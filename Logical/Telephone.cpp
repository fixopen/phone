...
Phone::Phone()
{
    ...
    ipBasAudio       =   NULL;
    pEvent = NULL;
    g_RingSound = -1000; //-(MAX_SOUND-MIN_SOUND)/2; 
    gRingFilename[0] = '\0';
    //end ring tone
...
void Phone::HungOn()
{
    ...
    while (dwSize == 0 && ncount++ < 5)
    {
        WriteFile(hPort, &value, 2, &dwSize,NULL);
        Sleep(50);
    }
}
...
void Phone::WritePhoneData()
{
    ...
    {
        WriteFile(hPort, dialcode_+writecurrentcount_, 1, &dwSize,NULL);
        writecurrentcount_++;
        Sleep(200);
    }
    if (writestatus_ == 0)
...
void Phone::PlayRingPrc(void)
{
    ...
    while (bRun)
    {
        Sleep(10);
        long evCode;
        if (main->phone_->pEvent && main->phone_->gRingFilename[0] != '\0' && main->phone_->nRingCount == 0xFF)  //循环播放
        {
