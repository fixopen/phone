...
namespace Data {
    ...
    std::vector<Util::shared_ptr<SoundSegment> > SoundSegment::GetFromDatabase(std::string const& filter, std::string const& orderFieldName, SeqenceRelation const dir, int const offset, int const pageSize) {
		return DataAccess<SoundSegment>::GetDatasByFilter(filter, modifyFieldByDB_, orderFieldName, dir, offset, pageSize);
	}
