...
void CMainDlg::SetDateTime(BOOL isDraw) {
    ...
    sprintf_s(time, 40, "%02d:%02d:%02d", curtime.wHour, curtime.wMinute, curtime.wSecond);
    sTime = time ;
    ...
    sprintf_s(txt, 64, "%04d-%02d-%02d", curtime.wYear, curtime.wMonth, curtime.wDay);
    sTime = txt;
    ...
    oldTime = sTime;
    sprintf_s(txt, 64, "%s", Data::LanguageResource::Get(Data::RI_COMN_SUNSTC + nWeekDay).c_str());
