...
LRESULT CDraftDlg::OnClickMJPG(WPARAM w, LPARAM l) {
    LRESULT result = 0;
...
LRESULT CDraftDlg::OnListCltrlClick(WPARAM w, LPARAM l) {
    LRESULT result = 0;
...
void CDraftDlg::Repeat()//重发
{
    ...
    if (GetFirstClickID(index)) {
        if (MMS_TYPE == m_nSMSType) {
            ...
            SYSTEMTIME tm;
              GetLocalTime(&tm);
              pmessge->timestamp.year = static_cast<unsigned char>(tm.wYear % 100);
              pmessge->timestamp.month = static_cast<unsigned char>(tm.wMonth);
              pmessge->timestamp.day = static_cast<unsigned char>(tm.wDay);
              pmessge->timestamp.hour = static_cast<unsigned char>(tm.wHour);
              pmessge->timestamp.minite = static_cast<unsigned char>(tm.wMinute);
              pmessge->timestamp.second = static_cast<unsigned char>(tm.wSecond);
