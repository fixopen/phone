...
BOOL CContactGroupDlg::OnInitDialog() {
    ...
    width = 226;
    height = 32;
    m_edtName.Create(WS_CHILD | WS_VISIBLE, CRect(xbegin, ybegin, xbegin + width, ybegin + height), this, IDC_EDIT_CONTACTGROUP_NAME);
    m_edtName.SetLimitText(15);

    ybegin = 158 + 105;
    m_ringEdit.Create(WS_CHILD | WS_VISIBLE, CRect(xbegin, ybegin, xbegin + 167, ybegin + height), this, 0xFFFF);
    m_ringEdit.SetIsAutoInput(FALSE);

    //按钮
    m_ringButton.Create(L"", Data::g_comboxBMPID[0][Data::g_skinstyle], Data::g_comboxBMPID[1][Data::g_skinstyle], WS_CHILD | WS_VISIBLE, CRect(xbegin + 167, ybegin, xbegin + 167 + 58, ybegin + height + 7), this, IDC_SETTING_RINGSELECT);

    m_ringList.Create(WS_CHILD | LVS_REPORT | LVS_NOCOLUMNHEADER | LVS_NOSORTHEADER, CRect(xbegin, ybegin + 38, xbegin + 226, ybegin + 38 + 150), this, IDC_SETTING_LSTRING, TRUE, 1);
    ...
    m_cmbSoundTip.Create(WS_CHILD | WS_VISIBLE, CRect(xbegin - 4, 205 + 105 - 5, xbegin + width, 205 + 105 - 5 + 153), this, IDC_COMBOBOX_CONTACTGROUP_SOUNDTIP);
...
void CContactGroupDlg::SetContactGroup(boost::shared_ptr<Data::ContactGroup> contactgroup) {
    ...
    for (size_t i = 0; i < sounds.size(); ++i) {
        ...
        } else if (sounds[i]->filename().substr(0, pos + 1) == ssFlashRecordPath) {
              m_cmbSoundTip.SetItemData(i + 1, 0);
        } else if (sounds[i]->filename().substr(0, pos + 1) == ssFlashRecordPath) {
              m_cmbSoundTip.SetItemData(i + 1, 0);
        } else {
...
void CContactGroupDlg::SetRingLst(TCHAR* dir) {
    ...
    if (wcslen(dir) == 0) {
        ...
        memcpy(m_ringDir, ptchLocalBell, wcslen(ptchLocalBell) * 2);
    } else {
