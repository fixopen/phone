...
void CScreenSaveDlg::SetSaveScreenPhoto_() {
    if (playerDlg_->SetImageListForScreenSave(my_photo_dir_list)) {
    } else {
...
void CScreenSaveDlg::OnTimer(UINT nIDEvent) {
    ...
    if (IDT_SCREENSAVE_TIME == nIDEvent) {
        if (type_ == 0) {
            ...
            sprintf_s(time, 24, "%02d:%02d:%02d", curtime.wHour, curtime.wMinute, curtime.wSecond);
            CString s = time;
            ...
            CFont m_Font;
            VERIFY(m_Font.CreateFont(64,                        // nHeight
                0,                         // nWidth
                0,                         // nEscapement
                0,                         // nOrientation
                FW_NORMAL,                 // nWeight
            ...
            sprintf_s(time, 24, "%02d-%02d-%02d %s", curtime.wYear, curtime.wMonth, curtime.wDay, Data::LanguageResource::Get(Data::RI_COMN_SUNSTC + nWeekDay).c_str());
            s = time;

            rt = CRect(nx, ny, nx + 210, ny + 28);
            CFont m_Font1;
            VERIFY(m_Font1.CreateFont(24,                        // nHeight
                0,                         // nWidth
                0,                         // nEscapement
                0,                         // nOrientation
                FW_NORMAL,                 // nWeight
