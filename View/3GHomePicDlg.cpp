...
LRESULT C3GHomePicDlg::OnClickMJPG(WPARAM w, LPARAM l) {
    LRESULT result = 0;
...
void C3GHomePicDlg::OnShiftFile() {
    ...
    for (size_t loop = 0; loop < m_fileCount; loop++) {
        if (m_isFileSelect[loop] == 1) {
            ...
            size_t convertCount = 0;
            int n = wcstombs_s(&convertCount, filename, 128, (LPTSTR)(LPCTSTR)m_filePicture[loop], 128);
            filename[n] = '\0';
...
LRESULT C3GHomePicDlg::OnDeviceChange(WPARAM w, LPARAM l) {
    LRESULT result = 0;
    if (w == 0x8000) //insert
    {
        ::Sleep(500);
...
void C3GHomePicDlg::OnOK_() {
    ...
    for (size_t i = m_fileDir.size(); i < m_isFileSelect.size(); i++) {
        if (1 == m_isFileSelect[i]) {
...
void C3GHomePicDlg::OnPlayer(int index) {
    ...
    CString s;
    s.Format(L"/%d", nFileSelected);
    main->m_pMainDlg->m_mainPhotoDlg_->m_MJPGList.SetUnitText(31, s, TRUE);
    main->m_pMainDlg->m_mainPhotoDlg_->m_MJPGList.SetUnitText(30, L"1", TRUE);
