...
BEGIN_MESSAGE_MAP(CHuangliDlg, CDialog)
    ...
    ON_MESSAGE(WM_MJPGMOVE, OnMJPGMOVE)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CHuangliDlg::OnInitDialog() {
    ...
    m_MJPGList.SetCurrentLinkFile(".\\adv\\mjpg\\k1\\中文\\老黄历.xml");
    ...
    default:
        if (6 <= w && w <= 47) {
            ...
            if (i >= nWeek && i < (nWeek + nDay)) {
                ...
                m_MJPGList.Invalidate();
                // 
                //                  m_MJPGList.SetUnitIsShow(nOldIndex+100, TRUE);
                //                  m_MJPGList.SetUnitIsShow(nOldIndex+200, TRUE);
                //                  m_MJPGList.SetUnitIsShow(nOldIndex+300, TRUE);
                //                  m_MJPGList.SetUnitIsShow(m_curIndex+100, TRUE);
                //                  m_MJPGList.SetUnitIsShow(m_curIndex+200, TRUE);
                //                  m_MJPGList.SetUnitIsShow(m_curIndex+300, TRUE);
            }
        }
        break;
    }
...
void CHuangliDlg::SetData(int y, int m, int d, BOOL flag) {
    ...
    sprintf_s(txt, 64, "%02d月", m_Month);
    CString sT = txt;
    m_MJPGList.SetUnitText(56, sT, FALSE);
    sprintf_s(txt, 64, "%04d年", m_Year);
    sT = txt;
    ...
    for (int i = 0; i < 42; i++) {
        ...
        if (i < nWeek) {
            ...
            if (ret) {
                ...
                sprintf_s(sBuff_, 64, "%d日%s", nn_, txt);
                sT = sBuff_;
                if (nn_ <= 15) {
                ...
                if (d == 1) {
                    ...
                    if (n > 0) {
                        if (iYear == m_Year && iMonth == m_Month && iDay == (i + 1 - nWeek)) {
                            isRunDayStart = TRUE;
                            sLunder = Data::LanguageResource::Get(Data::RI_LANUDER_LOOP).c_str();
                            sLunder += month[m - 1];
                        } else {
                            sLunder = month[m - 1];
                        }
...
void CHuangliDlg::ShowLunderRight(BOOL isDraw) {
    ...
    CString sT;
    //lxz 20081104
    sprintf_s(txt, 64, "%04d年%02d月", m_Year, m_Month);
    sT = txt;
    m_MJPGList.SetUnitText(49, sT, FALSE);

    sprintf_s(txt, 64, "%d", m_Day);
    sT = txt;
    m_MJPGList.SetUnitText(50, sT, FALSE);

    int nWeek = Logical::LunarderDate::WeekDay(m_Year, m_Month, m_Day);
    sprintf_s(txt, 64, "%s", Data::LanguageResource::Get(Data::RI_COMN_SUNSTC + nWeek).c_str());
    sT = txt;
    ...
    sprintf_s(txt, 64, "%s%s%s%s%s%s", Data::LanguageResource::Get(Data::RI_MAIN_LUNARCALENDARSTC).c_str(), chYear, Data::LanguageResource::Get(Data::RI_COMN_YEARSTC).c_str(), chLeapMonth, chMonth, chDay);
    sT = txt;
