...
double WTOF(CString buffer)//转浮点数
{
	char buf[32];
	memset(buf, 0, 32);
...
/////////////////////sprintf()//////////////////
void wsprintf_1(WCHAR *wstr, int num, double val)//多字节转宽字节
{
	char str[128]; 
	memset(str, 0, 128);
...
BOOL CCalculaterDlg::OnInitDialog() 
{
	...
	rectA[0] = CRect(56, 1, 104, 21*VHEIGHT);
	rectA[1] = CRect(108, 1, 156, 21*VHEIGHT);
	rectA[2] = CRect(160, 1, 208, 21*VHEIGHT);
	...
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < 6; j++)
		{
			rectA[i*6+j+3] = CRect(x_[j*2], y_[i*2]*VHEIGHT , x_[j*2+1], y_[i*2+1]*VHEIGHT);
		}
	CPoint p = CPoint(0, 0);
	CRect rect1 = CRect(17, 53*VHEIGHT, 17+212, (53+143)*VHEIGHT);
	...
	CRect r(21, 29*VHEIGHT, 225, 51*VHEIGHT);
...
// 	//defect 125
//     m_DialNumEdit.SetFocus();
//     m_DialNumEdit.ShowCaret();
//    //////////////
	CString s;
	s = Data::LanguageResource::Get(Data::RI_MAIN_CALCULATORBTN).c_str();
	m_stcTitle.Create(s, WS_CHILD|WS_VISIBLE, CRect(21, 13*VHEIGHT, 81, 28*VHEIGHT), this);
//	m_stcTitle.SetBorder(TRUE);
	m_stcTitle.SetColor(RGB(0, 0, 0), Data::g_staticAppTileBackRGB[Data::g_skinstyle]);
	m_btnExit.Create(L"", Data::g_buttonExitBMP1ID[0][Data::g_skinstyle], Data::g_buttonExitBMP1ID[1][Data::g_skinstyle], WS_CHILD|WS_VISIBLE, CRect(212, 14*VHEIGHT, 212+15, 26*VHEIGHT), this, IDC_BUTTON_CALUEXIT);

	m_backGroudStatic.Create(CRect(0, 0, 240, 234*VHEIGHT), this);
	m_backGroudStatic.SetInRect(CRect(13, 9*VHEIGHT, 232, 198*VHEIGHT), 1);
	...
	else
	{
		...
		switch ( w ) 
		{
			...
			case FUHAO_KEYV :
				...
				wsprintf(buffer, _T("%16.14f"),temp2);
				for (i =  wcslen(buffer); i > 0; i --)
					if (buffer[i-1] != '0')break;
				if (buffer[i-1] == '.')
				{
			...
			case CHU_KEYV :				
				...
				else if (m_keyvalue2 == m_keynull)
				{
					//strcpy(buffer1, "除数不能为零");
					//m_DialNumEdit.SetWindowText(L"Error");
					m_DialNumEdit.SetWindowText((LPCTSTR)CString("除数不能为零"));
					m_IsError = TRUE;//2005.5.19 zmy 设置错误的标志
				}
...
LRESULT CCalculaterDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	switch(message)
	{
	case WM_DELETE:
...
LRESULT CCalculaterDlg::OnChangeLanguage(WPARAM wParam,LPARAM lParam)
{
  ChangeLanguage();
...
void CCalculaterDlg::ChangeLanguage()
{
	CString s;
	
	s = Data::LanguageResource::Get(Data::RI_MAIN_CALCULATORBTN).c_str();

	m_stcTitle.SetWindowText(s);
}

void CCalculaterDlg::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
