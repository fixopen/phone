	...
	case 3:
		...
		if(m_bIsSIM)
		{
			...
			if(!((CMultimediaPhoneDlg*)theApp.m_pMainWnd)->simdevice_->Reset())
				m_MJPGList.SetUnitText(2, "出错或卡不存在", TRUE);
		}
		
		m_procbar.SetPos(0);
		ShowWindow(SW_HIDE);
		m_MJPGList.SetUnitText(2, "", FALSE);
		{
...
BOOL CSIMImportDlg::OnInitDialog() 
{
	...
	m_Edit1.Create(WS_CHILD | ES_MULTILINE | ES_AUTOVSCROLL | ES_WANTRETURN | WS_VSCROLL, CRect(0, 250, 800, 400), this, IDC_EDIT1);
...
//	m_procbarSound.SetParam(0, 0, 50, 1);
	...
	m_MJPGList.SetCurrentLinkFile(".\\adv\\mjpg\\k1\\中文\\导入到类别.xml");
...
LRESULT CSIMImportDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if(message == WM_SIMDATA_OK)
	{
		...
		if(gCmdIndex > 6)
		{
			if(len <= 2)
			{
				m_MJPGList.SetUnitText(2, "卡无法读取", TRUE);
				return CCEDialog::WindowProc(message, wParam, lParam);
			}
			...
			if(strlen(telcode) > 0)
			{
				...
				CString sName_ = cName;
				CString sCode_ = telcode;
	...
	else if(message == WM_SIMDATA_ERROR)
	{
		...
		m_MJPGList.SetUnitText(2, "传输出错", TRUE);
...
//	m_Static1.ShowWindow(SW_SHOW);
	m_MJPGList.SetUnitText(2, "开始导入", TRUE);
	...
	default:
		{
			if(gIndex >= gRecordCount)
			{
				...
				m_MJPGList.SetUnitText(2, txt, TRUE);
...
void CSIMImportDlg::OnTimer(UINT nIDEvent) 
{
	...
	if(nIDEvent == 101)
	{
		...
		{
			m_MJPGList.SetUnitText(2, "导入失败", TRUE);
		}
	}
