...
void CSoundDlg::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
    if (nChar == 'U') {
...
BOOL CSoundDlg::OnInitDialog() {
    ...
    */  
    m_lsList.Create(WS_CHILD | WS_VISIBLE | LVS_REPORT | LVS_NOCOLUMNHEADER | LVS_NOSORTHEADER, CRect(115, 45, 115 + 572, 45 + 360), this, IDC_LIST_SOUND_LIST, TRUE, 0, FALSE);
    ...
    */
    ...
    m_MJPGList.SetCurrentLinkFile(".\\adv\\mjpg\\k1\\中文\\录音.xml");
...
void CSoundDlg::OnButtonSetTip() {
    ...
    if (pos != NULL) {
        ...
        if ((m_vCurrentResult[index]->type() == Data::sstLocalRecord)) {
            if ((m_vCurrentResult[index]->duration() <= 10)) {
...
void CSoundDlg::ScrollItemsInList(int step, int nPos) {
    ...
    m_vCurrentResult = Data::SoundSegment::GetFromDatabaseByTypeOffsetLength(filter, nPos, PageSize);
    // 
...
void CSoundDlg::ShowArrayInList(std::vector<boost::shared_ptr<Data::SoundSegment> > array, int type) {
    ...
    for (size_t i = 0; i < (array).size(); ++i) {
        ...
        temp.Format(_T("%02d-%02d %02d:%02d"), array[i]->startTime().GetMonth(), array[i]->startTime().GetDay(), array[i]->startTime().GetHour(), array[i]->startTime().GetMinute());
        ...
        if (!DetectFile((TCHAR *)(LPCTSTR)sTemp/*.GetBuffer(256)*/)) {
            temp = "不存在";
        }
...
void CSoundDlg::DeleteSelectedItem(void) {
    ...
    if (pos != NULL) {
        ...
        if (!m_vCurrentResult[index]->played()) {
            ::PostMessage(((CMultimediaPhoneDlg *)theApp.m_pMainWnd)->m_pMainDlg->GetSafeHwnd(), WM_TELNOTIFY, 2, 2);
        }
        bool isTip = m_vCurrentResult[index]->isTip();
...
void CSoundDlg::SaveSoundSegment(boost::shared_ptr<Data::SoundSegment> ss) {
    ...
    if (cout > SoundTotal) {
        std::vector<boost::shared_ptr<Data::SoundSegment> > temp = Data::SoundSegment::GetFromDatabase("", Data::dUp, 0, cout - SoundTotal);
        if (!temp.empty()) {
...
LRESULT CSoundDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_TEL_INUSE:
        if (!m_bTelephoneInUse) {
            ...
            if (m_pPlaySoundDlg->IsWindowVisible()) {
                m_pPlaySoundDlg->CloseSound();
            }
            if (m_pRecordSoundDlg->IsWindowVisible()) {
