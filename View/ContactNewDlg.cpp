...
BOOL CContactNewDlg::OnInitDialog() 
{
	...
	xbegin = 398;
	width =  233;
	m_edtCompany.Create(WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, 
		CRect(xbegin, ybegin, xbegin+width, ybegin+height), this, IDC_EDIT_CONTACTNEW_COMPANY);
	m_edtCompany.SetLimitText(30);
	
	//地址
	ybegin += yspace+height ;
	m_edtCompanyAddress.Create(WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, 
		CRect(xbegin, ybegin, xbegin+width, ybegin+height),this, IDC_EDIT_CONTACTNEW_ADDRESS);
	m_edtCompanyAddress.SetLimitText(30);
	
	//邮编
	ybegin += yspace+height ;
	m_edtPostCode.Create(WS_CHILD|WS_VISIBLE, 
		CRect(xbegin, ybegin, xbegin+width, ybegin+height),this, IDC_EDIT_CONTACTNEW_POSTCODE);
	m_edtPostCode.SetLimitText(6);
	m_edtPostCode.SetLimitDiagital();

	//邮件
	ybegin += yspace+height ;
	m_edtEmail.Create(WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL, 
		CRect(xbegin, ybegin, xbegin+width, ybegin+height),this, IDC_EDIT_CONTACTNEW_EMAIL);
...
//	CRect(xbegin, ybegin, xbegin+width, ybegin+height), this, IDC_COMBOBOX_SETTING_RINGTIMES);
	...
	m_edtDepartment.Create(WS_CHILD|ES_AUTOHSCROLL, 
		CRect(475-8, 271-4, 675-8, 301-4), this, IDC_EDIT_CONTACTNEW_DEPARTMENT);
	m_edtDepartment.SetLimitText(30);

	m_edtDuty.Create(WS_CHILD, 
		CRect(475-8, 320-4, 675-8, 350-4), this, IDC_EDIT_CONTACTNEW_DUTY);
