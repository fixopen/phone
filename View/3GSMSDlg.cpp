...
LRESULT CSMSDlg::OnClickMJPG(WPARAM w, LPARAM l) {
    LRESULT result = 0;
...
bool CSMSDlg::SenderIsFull() {
    ...
    if (!content[0].IsEmpty() && !content[1].IsEmpty() && !content[2].IsEmpty() && !content[3].IsEmpty() && !content[4].IsEmpty()) {
        return true ;
    }
    return false ;
}
...
void CSMSDlg::SetSender(std::vector<CString> telnum) {
    ...
    for (size_t i = 0 ; i < telnum.size() ; i++) {
...
void CSMSDlg::SendSMS()//短消息
{
    ...
    GetTelnum(current_page);
    std::string address ;
    for (size_t i = 0 ; i < m_mapTelnum.size(); i++) {
...
void CSMSDlg::SaveDraft() {
    ...
    if (!smsContent.IsEmpty() || m_mapTelnum.size() > 0) {
        ...
        SYSTEMTIME tm;
        GetLocalTime(&tm);
        pMessage->timestamp.year = static_cast<unsigned char>(tm.wYear % 100);
        pMessage->timestamp.month = static_cast<unsigned char>(tm.wMonth);
        pMessage->timestamp.day = static_cast<unsigned char>(tm.wDay);
        pMessage->timestamp.hour = static_cast<unsigned char>(tm.wHour);
        pMessage->timestamp.minite = static_cast<unsigned char>(tm.wMinute);
        pMessage->timestamp.second = static_cast<unsigned char>(tm.wSecond);
