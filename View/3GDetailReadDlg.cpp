    ...
    if (m_nMenuType == 0)       //天气预报
    {
        ...
        if (m_pWeatherdateresult.size() > 0) {
            ...
            for (int i = 0; i < 5 && i < m_nMenuCount; i++) {
                ...
                sprintf_s(txt, 32, "%s %04d年%d月%d日", m_pWeatherdateresult[i]->weatherInfos.area.c_str(), m_pWeatherdateresult[i]->weatherInfos.datetime.GetYear(), m_pWeatherdateresult[i]->weatherInfos.datetime.GetMonth(), m_pWeatherdateresult[i]->weatherInfos.datetime.GetDay());
    ...
    if (m_nMenuType == 0)           //天气
    {
        ...
        //  for(int i = 0; i < m_pWeatherresult.size(); i++)
        {
            ...
            */
            ...
            sprintf_s(txt, 32, "%04d年%d月%d日", m_pWeatherdateresult[i]->weatherInfos.datetime.GetYear(), m_pWeatherdateresult[i]->weatherInfos.datetime.GetMonth(), m_pWeatherdateresult[i]->weatherInfos.datetime.GetDay());
            weather += txt;
            weather += "\r\n";
        ...
        if (m_pMediaFileresult[i]->multimediaInfos.type == Data::MultimediaDownload::MultimediaInfo::tText) {
            //todo 文字
            m_onlyEdit.ShowWindow(SW_SHOW);
        ...
        } else if (m_pMediaFileresult[i]->multimediaInfos.type == Data::MultimediaDownload::MultimediaInfo::tImage) {
              //todo 图片
              m_MJPGList.SetUnitBitmap(16, m_pMediaFileresult[i]->multimediaInfos.filename.c_str(), "", flag, TRUE);
        } else if (m_pMediaFileresult[i]->multimediaInfos.type == Data::MultimediaDownload::MultimediaInfo::tMMS) {
              //todo 彩信
