...
LRESULT CMainPhotoDlg::OnListCltrlClick(WPARAM w, LPARAM l)
{
	LRESULT ret;
...
BOOL CMainPhotoDlg::OnInitDialog() 
{
	...
	*/
	...
	m_MJPGList.SetCurrentLinkFile(".\\adv\\mjpg\\k1\\中文\\相册1.xml");
...
int CMainPhotoDlg::SetPlayList(TCHAR *dir, int local)
{
	...
	if(m_nPhotoType == net_type)   //网络相片
	{
		...
		if(filecount > 0);
		{
			for(int i = 0; i < filecount; i++)
			{
				...
				m_lstPlayList.SetItemText(i, 1, L"删除");
	...
	else 
	{
		...
		if(/*strstr(filename, ".bmp")||strstr(filename, ".BMP") ||*/ strstr(filename, ".jpg")||strstr(filename, ".JPG"))
		{
			...
			//	m_lstPlayList.SetItemText(ncount, 1, L"删除");
...
//播放
void CMainPhotoDlg::OnPlayer_(int index)
{
	CMultimediaPhoneDlg* main = (CMultimediaPhoneDlg*)theApp.m_pMainWnd;
