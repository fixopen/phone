...
void CContactInfoDlg::SaveContactInfo(boost::shared_ptr<Data::ContactInfo> ci) {
    ...
    //m_uiSelectIndex = 0;
    //ShowItemsInList(0);
    size_t count = Data::ContactInfo::GetDataCount("");
...
void CContactInfoDlg::SelectAll() {
    ...
    if (!m_bSelectAll) {
        m_bSelectAll = true ;
        for (size_t i = 1 ; i <= m_vCurrentResult.size() ; i++) {
            unitID = i * 100 ;
...
void CContactInfoDlg::ShowTypeInfo() {
    ...
    SetUpPages();
    for (size_t i = 0; i < m_vCurrentResult.size(); ++i) {
        boost::shared_ptr<Data::ContactInfo> result = m_vCurrentResult[i];
        ...
        temp.Format(_T("%02d/%02d/%02d"), result->startTime().GetYear(), result->startTime().GetMonth(), result->startTime().GetDay());
        ...
        if (result->duration() > 3600) {
            temp.Format(_T("%d:%02d:%02d"), result->duration() / 3600, result->duration() % 3600 / 60, result->duration() % 60);
        } else {
              temp.Format(_T("%d:%02d"), result->duration() / 60, result->duration() % 60);
        }
