...
: CDialog(CLunarderDlg::IDD, pParent) {
    ...
    VERIFY(m_font.CreateFont(24,                        // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
...
BOOL CLunarderDlg::OnInitDialog() {
    ...
    m_dtDateShow.Create(WS_CHILD | DTS_SHORTDATEFORMAT, CRect(43, 37, 262, 90), this, IDC_DATE_SHOW, 30, CDateCtrl::modeDate);
    m_dtTimeShow.Create(WS_CHILD | DTS_TIMEFORMAT, CRect(339, 37, 339 + 219, 90), this, IDC_TIME_SHOW, 30, CDateCtrl::modeTime);

    m_dtDate.Create(WS_CHILD | DTS_SHORTDATEFORMAT, CRect(41, 30, 197, 84), this, IDC_SETTING_DATE, 28, CDateCtrl::modeDate);
    m_dtTime.Create(WS_CHILD | DTS_TIMEFORMAT, CRect(261, 30, 417, 84), this, IDC_SETTING_TIME, 28, CDateCtrl::modeTime);
...
BOOL CLunarderDlg::IsHaveAlarm(int y, int m, int d) {
    ...
    char buff[32];
    memset(buff, 0, 32);
    sprintf_s(buff, 32, "%04d%02d%02d000000", time.GetYear(), time.GetMonth(), time.GetDay());
    filter += buff;
    filter += "' AND '";
    memset(buff, 0, 32);
    sprintf_s(buff, 32, "%04d%02d%02d235959", time.GetYear(), time.GetMonth(), time.GetDay());
    filter += buff;
    filter += "'";
