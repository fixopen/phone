...
void C3GHomeJoyDlg::ShowItemsInList(int type) {
    ...
    if (type == 0) {
        SetPlayList(ptchLocalMp3, type);
    } else if (type == 1) {
          ShowRecord("");
          memset(m_chDir, 0, 128 * 2);
          memcpy(m_chDir, (wchar_t*)(LPCTSTR)csFlashRecordPath, wcslen((LPCTSTR)csFlashRecordPath) * 2) ;
    } else if (type == 2) {
        if (1 == m_cardType2) {
            SetPlayList((wchar_t*)(LPCTSTR)csUsbDisk, type);
        } else if (2 == m_cardType2) {
            SetPlayList((wchar_t*)(LPCTSTR)csStarageCard, type);
        }
    } else if (type == 3) {
        if (1 == m_cardType3) {
            SetPlayList((wchar_t*)(LPCTSTR)csUsbDisk, type);
            ...
            if (m_fileMp3.begin() + i < m_fileMp3.end()) {
                ...
                if (i < m_fileDir.size()) {
                    ...
                    if (1 == m_uiType) {
                        ...
                        temp.Format(_T("%04d-%02d-%02d %02d:%02d"), m_vCurrentResult[i]->startTime().GetYear(), m_vCurrentResult[i]->startTime().GetMonth(), m_vCurrentResult[i]->startTime().GetDay(), m_vCurrentResult[i]->startTime().GetHour(), m_vCurrentResult[i]->startTime().GetMinute());
...
void C3GHomeJoyDlg::OnShiftFile() {
    ...
    for (size_t loop = 0; loop < m_fileCount; ++loop) {
        if (m_isFileSelect[loop] == 1) {
            ...
            size_t convertCount = 0;
            int n = wcstombs_s(&convertCount, filename, 128, (LPTSTR)(LPCTSTR)m_fileMp3[loop], 128);
            filename[n] = '\0';
...
LRESULT C3GHomeJoyDlg::OnDeviceChange(WPARAM w, LPARAM l) {
    LRESULT result = 0;
    if (w == 0x8000) //insert
    {
        ::Sleep(500);
...
LRESULT C3GHomeJoyDlg::OnRename(WPARAM w, LPARAM l) {
    LRESULT result = 0;
    CString s;
