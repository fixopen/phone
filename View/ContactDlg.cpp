...
LRESULT CContactDlg::OnClickMJPG_Toggle(WPARAM w, LPARAM l) {
    LRESULT result = 0;
    return result;
}
...
LRESULT CContactDlg::OnSoftKeyMessage(WPARAM wParam, LPARAM lParam) {
    ...
    default:
        ...
        GetDlgItem(IDC_STATIC_CONTACT_PINYIN)->SetWindowText(py + static_cast<wchar_t>(key));
        break;
    }
...
void CContactDlg::SelectAll() {
    ...
    if (!m_bSelectAll) {
        m_bSelectAll = true ;
        for (size_t i = 0 ; i < m_vCurrentResult.size() ; i++) {
...
void CContactDlg::HandleSimImport(WPARAM wp) {
    ...
    //SIM卡的容量
    size_t capacity = main->m_pATCommandWarp1->GetSimCapacity();
    ...
    {
        for (size_t i = 0 ; i < simresult.size() ; i++) {
    ...
    for (size_t i = 0 ; i < m_vClick.size() ; i++)//获得姓名 和 电话
    {
        if (1 == m_vClick[i]) {
...
void CContactDlg::ShowNewContact(int groupID) {
    ...
    if (0 == groupID)//全部
    {
        ...
        size_t i;
        for (i = 0 ; i < m_vGroupResult.size(); i++) {
    ...
    if (1 == m_iUpCurrentPage) {
        if (0 == index) {
            ...
            m_iCurrentGroupID = m_iSimID;
              ...
              m_sListFilter = "groupId = " + Util::StringOp::FromInt(m_vGroupResult[index - m_iBindCount]->id());
              m_iCurrentGroupID = m_vGroupResult[index - m_iBindCount]->id();
        }
    }

    if (m_iUpCurrentPage > 1) {
        m_sListFilter = "groupId = " + Util::StringOp::FromInt(m_vGroupResult[PageSize - m_iBindCount + (m_iUpCurrentPage - 2) * PageSize + index]->id());

        m_iCurrentGroupID = m_vGroupResult[PageSize - m_iBindCount + (m_iUpCurrentPage - 2) * PageSize + index]->id();
    }
}
void CContactDlg::ShowGroupInfo() {
    ...
    for (size_t i = 0; i < m_vCurrentResult.size(); ++i) {
        ...
        boost::shared_ptr<Data::Contact> result = m_vCurrentResult[i];
