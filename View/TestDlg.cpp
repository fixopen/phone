...
void CTestDlg::OnBtnTestDb() 
{
	...
	if (!m_bFlagTestDb)
	{
		if (!m_bFlagCall)
		{
			...
			m_bFlagTestDb = TRUE;
			DWORD dwThread = 0;
			HANDLE hThread = CreateThread(NULL, 0, ThreadTestDbProc, \
				reinterpret_cast<LPVOID>(this), 0, &dwThread);
		}
	}
...
DWORD CTestDlg::ThreadCallProc(LPVOID lParam)
{
	...
	CTelephoneDlg *PhoneDlg = \
		reinterpret_cast<CTelephoneDlg*>(((\
		CMultimediaPhoneDlg*)theApp.m_pMainWnd)->m_pTelephoneDlg);

	CSettingDlg *pSetDlg = reinterpret_cast<CSettingDlg*>\
		(((CMultimediaPhoneDlg*)theApp.m_pMainWnd)->m_pSettingDlg);
	std::map<char, std::string>  ms \
		=pSetDlg->m_pSetting->speedDials();
...
//		Writelog(ptcFileCall, strInfo);
		...
		main->m_pTelephoneDlg->DialContact(ms[1]);
...
{
	CTestDlg *pDlg = reinterpret_cast<CTestDlg*>(lParam);
	CMultimediaPhoneDlg *pMainDlg = \
		reinterpret_cast<CMultimediaPhoneDlg*>(theApp.m_pMainWnd);
...
//	Writelog(ptcFileDbTest, strInfo);
	...
	while (pDlg->m_bFlagTestDb/* && i < 1*/)
	{
		...
		while(pDlg->m_bFlagTestDb && nCount<nRange)
		{
			...
			//
			CString temp = Util::StringOp::ToCString(pContact->name());
			//
...
void CTestDlg::OnTimer(UINT nIDEvent) 
{
	if (TIMER_MEMORY == nIDEvent)
	{
		...
		strMemInfo.Format(_T("memory load: %u%%, total physical: %u(B),\
	...
	{
		CTelephoneDlg *PhoneDlg = reinterpret_cast<CTelephoneDlg*>\
			(((CMultimediaPhoneDlg*)theApp.m_pMainWnd)->m_pTelephoneDlg);
		KillTimer(1);
