...
namespace Util {
    std::wstring DataAccess<Telephone::Data::CallInfo>::tableName_ = L"contactInfo";
...
namespace Telephone {
    namespace Data {
        ...
        std::vector<Util::shared_ptr<Contact::Data::SoundSegment> > const CallInfo::sounds() const {
            if (hasSound_) {
                if (!soundsFinded_) {
                    sounds_ = Contact::Data::SoundSegment::Select(L"[contactInfoId] = " + Util::StringOp::FromInt(id()));
                    soundsFinded_ = true;
                }
            }
            return sounds_;
        ...
        void CallInfo::appendSound(Util::shared_ptr<Contact::Data::SoundSegment>& soundSegment) {
        ...
        Util::shared_ptr<Contact::Data::SoundSegment> const CallInfo::Record() {
            Insert();
            isInserted_ = true;
            Util::shared_ptr<Contact::Data::SoundSegment> result(new Contact::Data::SoundSegment());
            appendSound(result);
            ...
            if (Util::StoragePath::HasSdCard()) {
                ...
                Contact::Data::SoundSegment::remainSecondCount = freeBytes.QuadPart / secondBytes + 1;
            } else {
                ...
                Contact::Data::SoundSegment::remainSecondCount = (freeBytes.QuadPart - (totalBytes.QuadPart * 0.1)) / secondBytes + 1;
            }
            switch (type_) {
        case citMissed:
        ...
        default:
            ...
            result->filename(path + L"/" + Util::WaveRecorder::Instance()->GetFilename(now, true));
            if (Util::WaveRecorder::Instance()->IsRecording()) {
                Util::WaveRecorder::Instance()->Stop();
            }
            Util::WaveRecorder::Instance()->Start(result->filename());
            //result->Insert();
            return result;
        }
