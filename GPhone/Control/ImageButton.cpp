...
namespace Control {
    ...
    ImageButton::ImageButton(std::wstring const& imageFilename, SIZE const& imageSize)
    : CButton()
    ...
    void ImageButton::SetImage(std::wstring const& imageFilename, SIZE const& imageSize) {
        imageFilename_ = imageFilename;
        imageSize_ = imageSize;
    }

    void ImageButton::SetImage(std::wstring const& imageFilename, SIZE const& imageSize, std::wstring const& maskFilename) {
        imageFilename_ = imageFilename;
        imageSize_ = imageSize;
    ...
    void ImageButton::DrawItem(LPDRAWITEMSTRUCT lpDIS) {
        //draw background
        HDC commonButtonDc = Util::ImageOp::GetImage(imageFilename_, imageSize_);
        if (maskFilename_ == L"") {
            ::BitBlt(lpDIS->hDC, 0, 0, imageSize_.cx, imageSize_.cy, commonButtonDc, 0, 0, SRCCOPY);
        } else {
            HDC backgroundDc = Util::ImageOp::GetImage(L"/FlashDrv/debug/NULL.jpg", Util::Screen::size());
            RECT bounds;
            GetWindowRect(&bounds);
            ::BitBlt(lpDIS->hDC, 0, 0, imageSize_.cx, imageSize_.cy, backgroundDc, bounds.left, bounds.top, SRCCOPY);
            HDC maskDc = Util::ImageOp::GetMaskImage(maskFilename_, imageSize_);
            ::BitBlt(lpDIS->hDC, 0, 0, imageSize_.cx, imageSize_.cy, commonButtonDc, 0, 0, SRCINVERT);
            ::BitBlt(lpDIS->hDC, 0, 0, imageSize_.cx, imageSize_.cy, maskDc, 0, 0, SRCAND);
            ::BitBlt(lpDIS->hDC, 0, 0, imageSize_.cx, imageSize_.cy, commonButtonDc, 0, 0, SRCINVERT);
            //HBITMAP mask = Util::ImageOp::GetMaskImage(maskFilename_, imageSize_);
            //::MaskBlt(lpDIS->hDC, 0, 0, imageSize_.cx, imageSize_.cy, commonButtonDc, 0, 0, mask, 0, 0, SRCCOPY);
        }
