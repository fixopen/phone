        ...
        public:
            ...
            Telephone::Data::CallInfo const* const callInfo() const {
                switch (type_) {
                    ...
                    case sstCallRecord:
                        if (!callInfoFinded_) {
                            std::vector<Util::shared_ptr<Telephone::Data::CallInfo> > t = Telephone::Data::CallInfo::Select(L"id = " + Util::StringOp::FromInt(callInfoId_));
                            if (t.size() == 1) {
                                callInfo_ = t[0].get();
                            }
            ...
            Contact const* const contact() const {
                if (!contactFinded_) {
                    std::vector<Util::shared_ptr<Contact> > t = Contact::Select(L"id = " + Util::StringOp::FromInt(callInfo_->contactId()));
                    if (t.size() != 0) {
                        contact_ = t[0].get();
                    }
                    contactFinded_ = true;
