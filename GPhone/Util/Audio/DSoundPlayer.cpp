...
namespace Util {
    namespace Audio {
        DSoundPlayer* const DSoundPlayer::Instance() {
            static DSoundPlayer* result = 0;
            if (!result) {
                result = new DSoundPlayer();
                result->start();
            }
            return result;
        }

        int const DSoundPlayer::run() {
            while (!isQuit_) {
                if (isActive_) {
                    if (mediaControl_ && graphBuilder_ && basicAudio_ && mediaEvent_ && (filename_ != L"") && loopCount_) {
                        //循环播放
                        int loopCount = 0;
...
#if 0
        void DSoundPlayer::threadFunction_() {
        ...
        void DSoundPlayer::Start(std::wstring const & filename, int loopCount) {
            filename_ = filename;
            if (filename_ == L"") {
                Stop();
            } else {
        ...
        void DSoundPlayer::Stop(bool const release) {
            ::EnterCriticalSection(&criticalSection_);
            isActive_ = false;
            mediaControl_->Stop();
            if (playing_) {
        ...
        //-8000 ~ 0
        void DSoundPlayer::SetVolume(int const value) {
            int v = value;
            if (v <= -8000) {
                v = -8000;
            } else if (v > 0) {
        ...
        void DSoundPlayer::DecreaseVolume(int const offset) {
            if (basicAudio_) {
                volume_ -= offset;
        ...
        void DSoundPlayer::IncreaseVolume(int const offset) {
            if (basicAudio_) {
                volume_ += offset;
        ...
        void DSoundPlayer::SetMinVolume() {
            if (basicAudio_) {
                volume_ = MIN_SOUND;
                basicAudio_->put_Volume(volume_);
            }
        ...
        void DSoundPlayer::SetMaxVolume() {
            if (basicAudio_) {
                volume_ = MAX_SOUND;
                basicAudio_->put_Volume(volume_);
            }
        ...
        void DSoundPlayer::SetDefaultVolume() {
            if (basicAudio_) {
