...
void _yuv420ToRgb565(unsigned char const* const p_lum, unsigned char const* const p_cb, unsigned char const* const p_cr, int w_src, int h_src, unsigned char* const dest, int w_dst, int h_dst, int topdown) {
    int i, j;
    int y1, y2, u, v;
    ...
    default:
        // Down -> Top
        ptrarr[0] = ((unsigned short*)dest) + (w_dst * (h_dst - 1));
        for (j = 1; j < height; j++) {
            ptrarr[j] = ptrarr[j - 1] - w_dst;
        }
        break;
