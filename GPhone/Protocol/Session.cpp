#include "Session.h"
#include "../Util/Http/Communicator.h"
//#include "../Util/Digest/MD5/md5.h"
#include "../Util/Text/StringOp.h"
#include "../Util/Digest/SHA1/sha1.h"
#include "../Util/Encript/rsaeuro/source/rsa.h"
#include "../Util/BigInteger/BigUnsignedInABase.hh"
#include "../Util/BigInteger/BigIntegerAlgorithms.hh"
#include "../Util/BigInteger/BigIntegerUtils.hh"
...
namespace Protocol {
    ...
    void Session::Process(std::string const & body) {
        /*Communicator* */communicator_ = new Util::Http::Communicator();
        //c->ConsultKey("deviceId", "platformPublicKey");
        Util::Http::RequestMessage request;
        //request.type = RequestMessage::tGet;
        request.uri = "http://" + alterServer_() + "/" + resourceName_() + "?msisdn_d=" + msisdn_d_() + "&ua=" + userAgent_ + "&os=" + os_ + "&version=" + version_ + additionParameter_();
        request.HeaderFields.insert(std::make_pair(std::string("Content-Length"), Util::Text::StringOp::ToUTF8(Util::Text::StringOp::FromInt(body.length()))));
        request.Body = body;
        //request.version = "HTTP/1.1";
        //request.HeaderFields.insert(std::make_pair("x-device-id", deviceId_()));
        Util::Http::ResponseMessage response = communicator_->Get(request);
        if (response.state == Util::Http::ResponseMessage::sUnauthorized) {
            ...
            std::vector<std::string> bigParts = Util::Text::StringOp::Split(response.HeaderFields["WWW-Authenticate"], " ");
            //bigParts[0]; //method == Digest
            std::vector<std::string> parts = Util::Text::StringOp::Split(bigParts[1], ",");
            std::map<std::string, std::string> values;
            for (size_t i = 0; i < parts.size(); ++i) {
                std::vector<std::string> nameValuePair = Util::Text::StringOp::Split(parts[i], "=");
                values.insert(std::make_pair(nameValuePair[0], nameValuePair[1]));
            }
            response = auth_(values, request.uri);
        } else if (response.state == Util::Http::ResponseMessage::sRequestTimeout) {
              response = timeout_();
        } else {
    ...
    Util::Http::ResponseMessage const Session::auth_(std::map<std::string, std::string>& values, std::string const & digestUriValue) const {
        Util::Http::ResponseMessage result;
        ...
        sprintf_s(buffer, 9, "%08d", nc_);
        std::string nc = buffer;

        std::string cnonceValue = "1234567890";

        std::string responseValue = base16_(md5_(base16_(md5_(username + ":" + unquote_(values["realm"]) + ":" + msisdn_d_() + "123456")) + ":" + unquote_(values["nonce"]) + ":" + nc + ":" + cnonceValue + ":" + unquote_(values["qop"]) + ":" + base16_(md5_("GET:" + digestUriValue))));

        std::string authInfo = "Digest username=\"" + username + "\",realm=" + values["realm"] + ",nonce=" + values["nonce"] + ",uri=\"" + digestUriValue + "\",qop=" + values["qop"] + ",nc=" + nc + ",cnonce=\"" + cnonceValue + "\",response=\"" + responseValue + "\",opaque=" + values["opaque"];
        Util::Http::RequestMessage request;
        ...
        std::vector<std::string> parts = Util::Text::StringOp::Split(result.HeaderFields["WWW-Authenticate"], ",");
        values.clear();
        for (size_t i = 0; i < parts.size(); ++i) {
            std::vector<std::string> nameValuePair = Util::Text::StringOp::Split(parts[i], "=");
            values.insert(std::make_pair(nameValuePair[0], nameValuePair[1]));
    ...
    Util::Http::ResponseMessage const Session::timeout_() const {
        Util::Http::ResponseMessage result;
        return result;
    }

    void Session::selfProcess_(Util::Http::ResponseMessage const & response) const {
    }
    ...
    std::string const Session::getParameters_() const {
        std::string result;
        bool isFirstLoop = true;
        for (std::map<std::string, std::string>::const_iterator i = parameters_.begin(); i != parameters_.end(); ++i) {
            if (!isFirstLoop) {
                result += "&";
            } else {
                isFirstLoop = false;
    ...
    std::string const Session::sha1_(std::string const & value) const {
        ...
        if (err) {
            ...
            for (int i = 0; i < 20 ; ++i) {
                ...
                sprintf_s(data, 3, "%02X", Message_Digest[i]);
    ...
    std::string const Session::base16_(std::string const & value) const {
        ...
        for (size_t i = 0; i < value.length(); ++i) {
            ...
            sprintf_s(buffer, 3, "%02x", static_cast<unsigned char>(value[i]));
            result += buffer;
        }
        return result;
    }
    ...
    std::string const Session::rsaEncript_(R_RSA_PUBLIC_KEY const * const platformPublicKey, std::string const & deviceId, std::string const & randomKey) const {
        ...
        //return Util::StringOp::ToBase16(std::string((char*)output, outputLen));
    ...
    std::string const Session::alterServer_() const {
        return server_;
    }
}
