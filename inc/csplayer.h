...
int plyInit(
			HWND hWnd,
			MediaType mt				/* [IN] Handle to the window, will receive the heartbeat message */
			);
...
int plyOpen(
			LPTSTR pszFile,
			MediaType mt			/* [IN] Pointer to a null-terminated string that specifies the name of the file, to open */
			);
...
int plyPlay(
			BOOL fstart,
			MediaType mt				/* [IN] Specifies whether playback or pause */
			);
...
int plySetFullScreen(
					 BOOL isFullScreen,
					 MediaType mt	/* [IN] Specifies whether full screen or not */
					 );
...
int plySetPos(
			  int percent,
			  MediaType mt	/* [IN] percent/TRACKMAX 的值是百分比，比如要设置到 %5 的位置，则 percent = TRACKMAX * %5 */
			  );
...
int plySetRotate(
				 int rotate,
				 MediaType mt			/* [IN] Specifies how the window is to be rotated */
#define ROTATE_GUI					1
#define ROTATE_0					0
#define ROTATE_90					90
...
int plySetZoom(
			   int zoom,
			   MediaType mt				/* [IN] Specifies how the window is to be zoom */
...
int plyGetMediaInfo(
					LPMEDIA_INFO info,
					MediaType mt	/* [IN/OUT] Address of a MEDIA_INFO structure */
					);
...
int plySetRepeat(
				 BOOL repeat,
				 MediaType mt			/* [IN] Specifies whether repeat */
				 );
...
int plySetShuffle(
				  BOOL shuffle,
				  MediaType mt			/* [IN] Specifies whether shuffle */
				  );
...
int plySetAutoPreRotate(
						BOOL rotate,
						MediaType mt		/* [IN] Specifies whether auto prerotate */
						);
...
int plySetVideoOutput(
					  BOOL vid,
					  MediaType mt			/* [IN] Specifies whether video output */
					  );
...
int plySetAudioOutput(
					  BOOL aud,
					  MediaType mt			/* [IN] Specifies whether audio output */
					  );
...
int plySetMute(
			   BOOL mute,
			   MediaType mt				/* [IN] Specifies whether mute */
			   );
...
int plySetVolume(
				 int vol,
				 MediaType mt				/* [IN] Specifies the volume */
				 );
...
int plySetAspect(
				 int aspect,
				 MediaType mt				/* [IN] Specifies the aspect */
...
int plySetStereo(
				 int stereo,
			     MediaType mt				/* [IN] Specifies the stereo */
