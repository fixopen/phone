#include <objbase.h> //for next include
#include <INITGUID.h> //for init include next file : imaging.h's guid init
...
namespace Util {
    namespace Video {
        ...
        void ImageOp::drawImage_(std::wstring const & imageFilename, Size const & imageSize) {
            ...
            if (imageFactory) {
                ...
                if (image) {
                    ...
                    HBITMAP bitmap = ::CreateCompatibleBitmap(desktopDc, imageSize.width, imageSize.height);
        ...
        void ImageOp::drawMask_(std::wstring const & imageFilename, Size const & imageSize) {
            ...
            //FILE* file = 0;
            //fopen_s(&file, Util::Text::StringOp::ToUTF8(imageFilename).c_str(), "rb");
            if (file) {
        ...
        void ImageOp::draw_(IImagingFactory* const imageFactory, std::wstring const & imageFilename, Size const & imageSize) {
            ...
            if (image) {
                ...
                bounds.right = imageSize.width;
                bounds.bottom = imageSize.height;
                /*long */HRESULT r = image->Draw(dc, &bounds, 0);
                if (r != S_OK) {
                    DWORD errorCode = ::GetLastError();
                    DWORD ec = errorCode;
                }
