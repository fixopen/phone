...
namespace Util {
    ...
    public:
        ...
        static std::vector<Util::shared_ptr<T> > const GetDatasByFilter(std::wstring const filter, ModifyFieldByDB modifyFieldByDB, std::wstring const& orderFieldName = L"id", SeqenceRelation const sr = srNull, int const offset = 0, int const pageSize = -1) {
            ...
            cmd += Text::StringOp::ToUTF8(tableName_);
            cmd += "]";
            if (filter != L"") {
                cmd += " WHERE ";
                cmd += Text::StringOp::ToUTF8(filter);
            }
            switch (sr) {
            case srAsc:
                if (orderFieldName != L"") {
                    cmd += " ORDER BY " + Text::StringOp::ToUTF8(orderFieldName) + " ASC";
                }
                break;
            case srDesc:
                if (orderFieldName != L"") {
                    cmd += " ORDER BY " + Text::StringOp::ToUTF8(orderFieldName) + " DESC";
                }
                break;
            //case srNull:
            //case srError:
        ...
        static Util::shared_ptr<T> const GetDataById(int id, ModifyFieldByDB modifyFieldByDB) {
            ...
            std::wstring filter = L"id = " + Text::StringOp::FromInt(id);
