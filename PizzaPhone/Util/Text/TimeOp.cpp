...
namespace Util {
    namespace Text {
        ...
        std::wstring const TimeOp::ToString(Timestamp const & timestamp, TimeOp::Part const part, std::wstring const & separtors) {
            ...
            switch (part) {
            case pDateTime:
                result = StringOp::FromInt(timestamp.GetYear()) + separtors[0] + StringOp::FromInt(timestamp.GetMonth()) + separtors[0] + StringOp::FromInt(timestamp.GetDay()) + separtors[1] + StringOp::FromInt(timestamp.GetHour()) + separtors[2] + StringOp::FromInt(timestamp.GetMinute()) + separtors[2] + StringOp::FromInt(timestamp.GetSecond());
                break;
            case pDate:
                result = StringOp::FromInt(timestamp.GetYear()) + separtors[0] + StringOp::FromInt(timestamp.GetMonth()) + separtors[0] + StringOp::FromInt(timestamp.GetDay());
                break;
            case pTime:
                result = StringOp::FromInt(timestamp.GetHour()) + separtors[0] + StringOp::FromInt(timestamp.GetMinute()) + separtors[0] + StringOp::FromInt(timestamp.GetSecond());
                break;
            default:
                break;
            }
