#ifndef __UTIL_AUDIO_WAVEIN_H__
#define __UTIL_AUDIO_WAVEIN_H__
...
namespace Util {
    namespace Audio {
        class WaveIn {
        public:
            typedef void (*DataProcessor)(void const * const data,
                                          size_t const length);
            static WaveIn* const Instance();
            WaveIn();
            ...
            void SetVolume(size_t const volume) {
                //waveInSetProperty(waveIn_, const GUID* pPropSetId, ULONG ulPropId, LPVOID pvPropParams, ULONG cbPropParams, LPVOID pvPropData, ULONG cbPropData);
            }
        private:
            HWAVEIN waveIn_;
