...
namespace Telephone {
    namespace Data {
        ...
        Util::shared_ptr<::Telephone::Data::Record> const CallInfo::Record() {
            ...
            result->filename(path + L"/" + L""/*Util::Audio::WaveRecorder::Instance()->GetFilename(now, true)*/);
            if (Util::Audio::WaveIn::Instance()->IsRecording()) {
                Util::Audio::WaveIn::Instance()->Stop();
            }
            Util::Audio::WaveIn::Instance()->Start(result->filename());
            //result->Insert();
            return result;
        }
