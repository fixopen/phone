...
#include "../Logical/PstnTelephone.h" //for PstnTelephone
#include "../Logical/AtTelephone.h" //for AtTelephone
#include "../../Contact/Data/Contact.h" //for Contact
#include "../../Contact/View/ContactDialog.h" //for contactForm
#include "../../Util/Properties.h" //for Properties
#include "../../Util/Text/TimeOp.h" //for TimeOp
#include "../../Util/Video/ImageOp.h" //for ImageOp
#include "../../Util/Video/Screen.h" //for Screen
#include "../../Util/Video/BackgroundImage.h" //for BackgroundImage
//#include "PlayingRecordsDialog.h"

namespace Telephone {
    namespace View {
        CallDialog* const callForm() {
            static CallDialog* result = 0;
            if (!result) {
                result = new CallDialog(0);
                result->Create(CallDialog::IDD, 0);
            }
            return result;
        }
        ...
        HWND const CallDialog::GetLocalDisplay() const {
            return GetDlgItem(IDC_STATIC_LOCAL_DISPLAY)->m_hWnd;
        }

        HWND const CallDialog::GetRemoteDisplay() const {
            return GetDlgItem(IDC_STATIC_REMOTE_DISPLAY)->m_hWnd;
        }

        // CallDialog message handlers

        void CallDialog::SetNumber(std::wstring const& number) {
            CWnd* control = GetDlgItem(IDC_STATIC_NUMBER);
            control->SetWindowTextW(number.c_str());
        }

        void CallDialog::SetName(std::wstring const& name) {
            //CWnd* control = GetDlgItem(IDC_STATIC_NAME);
            //control->SetWindowTextW(name.c_str());
        }

        void CallDialog::SetAddress(std::wstring const& address) {
            //CWnd* control = GetDlgItem(IDC_STATIC_ADDRESS);
            //control->SetWindowTextW(address.c_str());
        }

        void CallDialog::SetStartTime(Util::Timestamp const& time) {
            //CWnd* control = GetDlgItem(IDC_STATIC_START_TIME);
        ...
        void CallDialog::OnBnClickedButtonClose() {
            ...
            ClearDuration();
            Logical::AtTelephone::Instance()->VideoHangup();
            ShowWindow(SW_HIDE);
        }
        ...
        void CallDialog::OnBnClickedButtonReject() {
            //Logical::AtTelephone::Instance()->SoftHangup();
            //Logical::AtTelephone::Instance()->fireEvent(PstnTelephone::eHangup, 0);
            //Logical::AtTelephone::Instance()->ClearCall();
        }

        void CallDialog::OnBnClickedButtonListen() {
            //Logical::AtTelephone::Instance()->SoftPickup();
            //Logical::AtTelephone::Instance()->fireEvent(PstnTelephone::ePickup, 0);
            //Logical::AtTelephone::Instance()->SetConnectionType(Logical::AtTelephone::ctHandsfree);
        }

        void CallDialog::OnBnClickedButtonHangup() {
            //Logical::AtTelephone::Instance()->SoftHangup();
            //Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eHangup, 0);
            //Logical::AtTelephone::Instance()->ClearCall();
        }
        ...
        void CallDialog::OnBnClickedButtonMute() {
            Logical::AtTelephone::Instance()->Mute(true);
        }

        void CallDialog::OnBnClickedButtonStartRecord() {
            Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eStartRecord, 0);
        }

        void CallDialog::OnBnClickedButtonStopRecord() {
            Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eStopRecord, 0);
        }

        void CallDialog::dial_(wchar_t const keyCode) {
            //KillTimer(timers_[Logical::AtTelephone::tDial]);
            //timers_.erase(Logical::AtTelephone::tDial);
            //CString value;
            //CWnd* control = GetDlgItem(IDC_EDIT_NUMBER);
            //control->GetWindowTextW(value);
            //value += keyCode;
            ...
            //    Logical::AtTelephone::Instance()->SetContact(contacts[0]);
            //}
            //timers_.insert(std::make_pair(Logical::AtTelephone::tDial, SetTimer(Logical::AtTelephone::tDial, Logical::AtTelephone::tDial, 0)));
        }
        ...
        void CallDialog::callId_(Logical::CALLID_INFO* callId) {
            //KillTimer(timers_[Logical::AtTelephone::tCallId]);
            //timers_.erase(Logical::AtTelephone::tCallId);
            //SetNumber(Util::StringOp::ToUnicode(callId->number));
            //timers_.insert(std::make_pair(Logical::AtTelephone::tCallId, SetTimer(Logical::AtTelephone::tCallId, Logical::AtTelephone::tCallId, 0)));
        ...
        void CallDialog::connect_() {
            ...
            //timers_.insert(std::make_pair(Logical::AtTelephone::tConnect, SetTimer(Logical::AtTelephone::tConnect, Logical::AtTelephone::tConnect, 0)));
        ...
        void CallDialog::startRecord_() {
            ...
            //timers_.insert(std::make_pair(Logical::AtTelephone::tRecord, SetTimer(Logical::AtTelephone::tRecord, 1000, 0)));
        }
        ...
        void CallDialog::stopRecord_() {
            //KillTimer(timers_[Logical::AtTelephone::tRecord]);
            //timers_.erase(Logical::AtTelephone::tRecord);
        }

        void CallDialog::remoteHangup_() {
            //CWnd* control = GetDlgItem(IDC_STATIC_COMMUNICATION_STATUS);
            //control->SetWindowTextW(L"Remote Hangup!");
            //KillTimer(timers_[Logical::AtTelephone::tConnect]);
            //timers_.erase(Logical::AtTelephone::tConnect);
            //KillTimer(timers_[Logical::AtTelephone::tRecord]);
            //timers_.erase(Logical::AtTelephone::tRecord);
            //hasCallId_ = false;
        }
        ...
        void CallDialog::softDial_(std::wstring const& number) {
            //KillTimer(timers_[Logical::AtTelephone::tWaitConnect]);
            //timers_.erase(Logical::AtTelephone::tWaitConnect);
            ShowWindow(SW_SHOW);
            SetNumber(number);
            //timers_.insert(std::make_pair(Logical::AtTelephone::tWaitConnect, SetTimer(Logical::AtTelephone::tWaitConnect, Logical::AtTelephone::tWaitConnect, 0)));
        }
        ...
        void CallDialog::OnTimer(UINT_PTR nIDEvent) {
            //if (nIDEvent == timers_[Logical::AtTelephone::tDial]) {
            //    KillTimer(nIDEvent);
            //    timers_.erase(Logical::AtTelephone::tDial);
            //    if (Util::Properties::Get(L"autoRecord") == L"1") {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eForceRecord, 0);
            //    } else {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eRemotePickup, 0);
            //    }
            //} else if (nIDEvent == timers_[Logical::AtTelephone::tWaitReceive]) {
            //    KillTimer(nIDEvent);
            //    timers_.erase(Logical::AtTelephone::tWaitReceive);
            //    if (Util::Properties::Get(L"autoRecord") == L"1") {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eForceRecord, 0);
            //    } else {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eRemotePickup, 0);
            //    }
            //} else if (nIDEvent == timers_[Logical::AtTelephone::tRing]) {
            //    KillTimer(nIDEvent);
            //    timers_.erase(Logical::AtTelephone::tRing);
            //    //if (ringCount > Util::Properties::Get(L"delayCount")) {
            //    //    Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eLeaveWord, 0);
            //    //} else {
            //    //    ++ringCount;
            //    //}
            //} else if (nIDEvent == timers_[Logical::AtTelephone::tCallId]) {
            //    KillTimer(nIDEvent);
            //    timers_.erase(Logical::AtTelephone::tCallId);
            //    if (Util::Properties::Get(L"autoAnswer") == L"1") {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eLeaveWord, 0);
            //    } else {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eTimeout, 0);
            //    }
            //} else if (nIDEvent == timers_[Logical::AtTelephone::tWaitConnect]) {
            //    KillTimer(nIDEvent);
            //    timers_.erase(Logical::AtTelephone::tWaitConnect);
            //    if (Util::Properties::Get(L"autoRecord") == L"1") {
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eForceRecord, 0);
            ...
            //        Logical::AtTelephone::Instance()->fireEvent(Logical::AtTelephone::eRemotePickup, 0);
            //    }
            //} else if (nIDEvent == timers_[Logical::AtTelephone::tConnect]) {
            ...
            //} else if (nIDEvent == timers_[Logical::AtTelephone::tRecord]) {
        ...
        BOOL CallDialog::OnEraseBkgnd(CDC* pDC) {
            BOOL r = Util::Video::BackgroundImage::drawBackground(m_hWnd);
            if (!r) {
