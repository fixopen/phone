...
void CMJPGStatic::SetCurrentLinkFile(CString s)
{
	...
	DWORD offset = GetTickCount() - dwStart;   
	TRACE(L"ParseFileToMJPGList_ %d\n", offset);
	...
	if(m_currentMJPGList)
	{
		...
		for(int i = 0; i < m_currentMJPGList->items.size(); i++)
		{
			if(m_currentMJPGList->items[i]->unitparam.m_Ctrltype > ctrl_unit)
			{
				...
				case ctrl_edit:
					{
						CCERectEdit *pEdit = new CCERectEdit;
						if(pEdit)
						{
							pEdit->Create(WS_CHILD|WS_VISIBLE, m_currentMJPGList->items[i]->unitparam.m_Rect, this, m_currentMJPGList->items[i]->unitparam.m_nSiererNO);
						//	pEdit->SetLimitText(32);
						//	pEdit->SetLimitDiagital();
							m_currentMJPGList->items[i]->unitparam.m_pUnitWnd = (CWnd *)pEdit;
						}
					}
					break;
				...
				case ctrl_commbox:
					{
						CCEComboBox_  *pCombox = new CCEComboBox_ ;
						CRect rt = m_currentMJPGList->items[i]->unitparam.m_Rect;
						rt.bottom += 360;
						pCombox->Create(WS_CHILD|WS_VISIBLE, rt, this, m_currentMJPGList->items[i]->unitparam.m_nSiererNO);
						m_currentMJPGList->items[i]->unitparam.m_pUnitWnd = (CWnd *)pCombox;
					}
					break;
				case ctrl_datetime:
					{
						CRect rt = m_currentMJPGList->items[i]->unitparam.m_Rect;
						//rt.bottom += 100;
						CDateCtrl	*pDateCtrl = new CDateCtrl;
						pDateCtrl->Create(WS_VISIBLE|WS_CHILD|DTS_SHORTDATEFORMAT, rt, this, m_currentMJPGList->items[i]->unitparam.m_nSiererNO, 28, CDateCtrl::em_mode::modeDate);
						m_currentMJPGList->items[i]->unitparam.m_pUnitWnd = (CWnd *)pDateCtrl;
					}
				default:
					break;
				}
...
//	CreateMemDC();
	...
	extern DWORD g_dwStart;
	DWORD offset = GetTickCount() - g_dwStart;   
	TRACE(L"run end %d\n", offset);
...
CWnd *CMJPGStatic::GetUnitAttachWnd(int UnitNo)
{
	CWnd *pWnd = NULL;
	if(!m_currentMJPGList)
		return pWnd;
	for(int i = 0; i < m_currentMJPGList->items.size(); i++)
	{
		if(m_currentMJPGList->items[i]->unitparam.m_nSiererNO == UnitNo)
		{
			pWnd = m_currentMJPGList->items[i]->unitparam.m_pUnitWnd;
			break;
		}
	}
	return pWnd;
