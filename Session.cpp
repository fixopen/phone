...
#include "../Util/rsaeuro/source/rsa.h"
...
namespace Protocol {
    ...
    void Session::Process(std::string const& body) {
        /*Communicator* */communicator_ = new Util::Communicator();
        //c->ConsultKey("deviceId", "platformPublicKey");
        Util::RequestMessage request;
        ...
        Util::ResponseMessage response = communicator_->Get(request);
        if (response.state == Util::ResponseMessage::sUnauthorized) {
        ...
        } else if (response.state == Util::ResponseMessage::sRequestTimeout) {
            response = timeout_();
        } else {
    ...
    Util::ResponseMessage const Session::auth_(std::map<std::string, std::string>& values, std::string const& digestUriValue) const {
        Util::ResponseMessage result;
    ...
    Util::ResponseMessage const Session::timeout_() const {
        Util::ResponseMessage result;
        return result;
    }

    void Session::selfProcess_(Util::ResponseMessage const& response) const {
    }
    ...
    std::string const Session::rsaEncript_(R_RSA_PUBLIC_KEY const* const platformPublicKey, std::string const& deviceId, std::string const& randomKey) {
        ...
        //int R_GenerateBytes(unsigned char* block, unsigned int length, R_RANDOM_STRUCT* randomStruct);
