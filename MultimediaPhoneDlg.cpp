    ...
    case WM_TEL_STATUS:
        //电话状态
        {
            ...
            if (wParam != TEL_SIGNALQUALITY && wParam != TEL_NETTYPE) {
                ...
                if (1 == status) {
                    sprintf_s(txt, 64, ".\\adv\\mjpg\\k5\\common\\png\\G网%d.png", lParam);
                } else if (2 == status) {
                    sprintf_s(txt, 64, ".\\adv\\mjpg\\k5\\common\\png\\T网%d.png", lParam);
                }
    ...
    case WM_CHANGE_BATTERY:
        {
            //if(!phone_->m_BatteryStatus.isCharge)
            {
                ...
                sprintf_s(txt, 64, ".\\adv\\mjpg\\k5\\common\\png\\电池%d.png", phone_->m_BatteryStatus.batteryProccess);
        ...
        do {
            ...
            if (number_ != "") {
                ...
                if (ret == 0) {
                    ...
                    char* ResponseStatus[] = {
                        "", "未知错误", "服务拒绝", "消息格式错误", "发送地址错误", "消息未发现", "网络错误", "消息内容错误", "不支持的消息"
                    };
                    if (ret > 0 && ret < 8) {
                        sprintf_s(txt, 64, "彩信发送失败", ResponseStatus[ret]);
                    } else {
                        strcpy_s(txt, 64, "彩信发送失败");
                    }
                    CString s = txt;
