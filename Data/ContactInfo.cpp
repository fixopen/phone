...
namespace Data
{
	...
	std::vector<boost::shared_ptr<ContactInfo> > ContactInfo::\
		GetFromDatabaseByTypeOffsetLength(std::string type, \
		int const offset, int const pageSize)
    {
		std::string filter = type;
        return DataAccess<ContactInfo>::GetDatasByFilterAndPageLimit(\
			filter, modifyFieldByDB_, offset, pageSize);
    }
	...
	//add by yy 20111103
	void ContactInfo::Update(std::string filter)
	{	
		std::string cmd = "UPDATE ";
        cmd += tableName();
		cmd += filter;
    ...
    void ContactInfo::Remove(std::string filter)
    {
		...
		void ContactInfo::Closedb()
	{
		Data::CloseDB(DataAccess<ContactInfo>::db_);
	}
    
	void ContactInfo::Opendb()
	{
		DataAccess<ContactInfo>::db_=Data::OpenDB();
	}
    ...
    void ContactInfo::modifyFieldByDB_(int argc, char** argv, char** columnName, boost::shared_ptr<ContactInfo> item)
    {
		...
		item->duration_ = atoi(argv[Data::getIndexByName(argc, columnName, "duration")]+1);
	...
	void ContactInfoSoundSegmentMap::Closedb()
	{
		Data::CloseDB(DataAccess<ContactInfoSoundSegmentMap>::db_);	 
	}

	void ContactInfoSoundSegmentMap::Opendb()
	{
		DataAccess<ContactInfoSoundSegmentMap>::db_=Data::OpenDB();
	}
    ...
    void ContactInfoPictureMap::Insert()
    {
   	...
   	void ContactInfoPictureMap::Closedb()
	{
		Data::CloseDB(DataAccess<ContactInfoPictureMap>::db_);
	}
    void ContactInfoPictureMap::Opendb()
	{
		DataAccess<ContactInfoPictureMap>::db_=Data::OpenDB();
	}
