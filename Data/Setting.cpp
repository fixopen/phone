...
namespace Data {
    ...
    int DataAccess<Setting>::count_ = 0;
    ...
    std::vector<Util::shared_ptr<Setting> > Setting::GetFromDatabase(std::string const& filter, std::string const& orderFieldName, SeqenceRelation const dir, int const offset, int const pageSize) {
		return DataAccess<Setting>::GetDatasByFilter(filter, modifyFieldByDB_, orderFieldName, dir, offset, pageSize);
	}

	Util::shared_ptr<Setting> Setting::GetCurrentConfig() {
		std::vector<Util::shared_ptr<Setting> > t = GetFromDatabase("type = " + Util::StringOp::FromInt(stCurrent));
		//assert(t.size() == 1)
		return t[0];
	}

	Util::shared_ptr<Setting> Setting::GetDefaultConfig() {
		std::vector<Util::shared_ptr<Setting> > t = GetFromDatabase("type = " + Util::StringOp::FromInt(stDefault));
		//assert(t.size() == 1)
		return t[0];
	}

	Util::shared_ptr<Setting> Setting::GetNormalConfig() {
		std::vector<Util::shared_ptr<Setting> > t = GetFromDatabase("type = " + Util::StringOp::FromInt(stNormal));
		//assert(t.size() == 1)
		return t[0];
	}

	Util::shared_ptr<Setting> Setting::GetSpeedConfig() {
		std::vector<Util::shared_ptr<Setting> > t = GetFromDatabase("type = " + Util::StringOp::FromInt(stSpeed));
		//assert(t.size() == 1)
		return t[0];
	}

	Util::shared_ptr<Setting> Setting::GetSpaceConfig() {
		std::vector<Util::shared_ptr<Setting> > t = GetFromDatabase("type = " + Util::StringOp::FromInt(stSpace));
		//assert(t.size() == 1)
		return t[0];
	}
	...
	void Setting::modifyFieldByDB_(int argc, char** argv, char** columnName, Util::shared_ptr<Setting> item) {
