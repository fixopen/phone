...
namespace Data
{
	...
	public:
		Log(); //create
		~Log(); //destroy
		static std::vector<boost::shared_ptr<Log> > GetFromDatabase(std::string filter, Direction const dir = dNull, int const id = 0, int const pageSize = 0); //get from database
		void Update() const; //sync to database
		void Insert(); //insert new instance to database
		static void Remove(std::string filter); //delete form database
		std::string ToString(void);
	private:
		static void modifyFieldByDB_(int argc, char** argv, char** columnName, boost::shared_ptr<Log> item);
	};
}
