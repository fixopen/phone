...
namespace Data
{
    ...
    std::vector<boost::shared_ptr<ContactGroup> > ContactGroup::GetFromDatabase(std::string filter)
    {
        return DataAccess<ContactGroup>::GetDatasByFilter(filter, modifyFieldByDB_);
    }
    ...
    void ContactGroup::Insert()
    {
        ...
        cmd += tableName();
        cmd += " ( name, alias, useDefaultRing, ringId, useDefaultTips, tipsId, iconId ) VALUES ( '"; //useDefaultSMRing, smRingId, , parentId, isLeaf, isEnableAppendSubGroup
        cmd += name_;
        cmd += "', '";
        cmd += alias_;
        cmd += "', ";
