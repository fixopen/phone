...
namespace Data
{
    ...
    {
        for (int i = 0; i < sizeof langResource / sizeof langResource[0][0]; ++i)
        {
            LanguageResource lr;
            lr.id(langResource[gLangItem][i].id);
            lr.value(langResource[gLangItem][i].value);
            lr.Insert();
        }
    }
	...
	std::string LanguageResource::Get(int const id)
	{
        ...
        for (int i = 0; i < sizeof langResource / sizeof langResource[0][0]; ++i)
        {
            if (langResource[gLangItem][i].id == id)
            {
                result = langResource[gLangItem][i].value;
                break;
            }
        }
        return result;
	...
	void LanguageResource::Closedb(){

		Data::CloseDB(DataAccess<LanguageResource>::db_);
	}

	void LanguageResource::Opendb(){

		DataAccess<LanguageResource>::db_=Data::OpenDB();
	}
