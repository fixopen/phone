...
namespace View {
    ...
    BOOL CContactDlg::OnInitDialog() 
    {
        ...
        m_lsList.Create(WS_CHILD|WS_VISIBLE|LVS_REPORT|LVS_NOCOLUMNHEADER|LVS_NOSORTHEADER, CRect(115*X_XISHU, 46*Y_XISHU, (115+570)*X_XISHU, (46+302)*Y_XISHU), &m_MJPGList, IDC_LIST_CONTACT_LIST, TRUE, 0, FALSE);
        ...
        CRect rectKey[29];
        unsigned short/*UINT16 */uiKey[29];
        int offsetX = 0;
        int offsetY = 0;
        for (int j = 0; j < 26; ++j)
        {	
    ...
    std::string CContactDlg::GetPYIndex(void)
    {
        std::string alias;
        CString py;
        m_sticPinYin.GetWindowTextW(py);
        //CWnd* c = GetDlgItem(IDC_STATIC_CONTACT_PINYIN);
        //c->GetWindowText(py);
        //GetDlgItem(IDC_STATIC_CONTACT_PINYIN)->GetWindowText(py);
        if (!py.IsEmpty())
        {
