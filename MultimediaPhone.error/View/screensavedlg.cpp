...
#define SCREENSAVE_TIMER		5*1000
    ...
    {
        //关闭输入法
        ((CMultimediaPhoneDlg*)theApp.m_pMainWnd)->SwitchPanel_(0);
        /*
        ...
        */
        ...
        if(type > 1 || type < 0)
            type = 1;
        type_ = type;
        playerDlg_->ReSetWindowsRect(CRect(0*X_XISHU, 0*Y_XISHU, 800*X_XISHU, 480*Y_XISHU));
        if(type_)   //图片
        {
            ::SetCursorPos(799*Y_XISHU, 479*Y_XISHU);
    ...
    void CScreenSaveDlg::SetSaveScreenPhoto_()
    {
        ...
        if (playerDlg_->SetImageListForScreenSave((char*)photoFilename.c_str()))
        {
    ...
    BOOL CScreenSaveDlg::OnInitDialog() 
    {
        ...
        //SetSaveScreenType(0);
        m_static.Create(L"", WS_VISIBLE|WS_CHILD, CRect(0, 0, 200*X_XISHU, 64), this);
        m_static.SetFontSize(64);
        m_static1.Create(L"", WS_VISIBLE|WS_CHILD, CRect(0, 0, 200, 32), this);
        m_static1.SetFontSize(28);
    ...
    void CScreenSaveDlg:: OnExit()
    {
        ...
        else
        {
            ...
            CString str = Util::StringOp::ToCString(Data::LanguageResource::Get(Data::RI_SCREENSAVE_PASSWORDINPUT).c_str());
    ...
    void CScreenSaveDlg::OnTimer(UINT nIDEvent) 
    {
        ...
        if(IDT_SCREENSAVE_TIME == nIDEvent)
        {
            if(type_ == 0)
            {
                ...
                //CString s = time;
                CString s;
                s.Format(_T("%02d:%02d:%02d"), curtime.wHour, curtime.wMinute, curtime.wSecond);
