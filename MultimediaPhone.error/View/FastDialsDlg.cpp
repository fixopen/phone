...
namespace View {
    ...
    BOOL CFastDialsDlg::OnInitDialog() 
    {
        ...
        */	
        ...
        m_MJPGList.SetCurrentLinkFile(_T(".\\adv\\mjpg\\k1\\ÖÐÎÄ\\ËÙ²¦.xml"));
        ...
        for (int i = 0; i < 12; ++i)
        {
            if (i < 6)
            {
                m_edtNumber[i].Create(WS_CHILD|WS_VISIBLE, CRect((107 )*X_XISHU, (78 + i*32 )*Y_XISHU, (286 )*X_XISHU, 104 + (i*32 )*Y_XISHU), &m_MJPGList, IDC_EDIT_FASTDIALS_NUMBER01);
                m_edtNumber[i].SetLimitText(15);
                m_edtNumber[i].SetLimitDiagital();
            }
            else
            {
                m_edtNumber[i].Create(WS_CHILD|WS_VISIBLE, CRect((366 )*X_XISHU, (78 + (i-6)*32 )*Y_XISHU, (544 )*X_XISHU, (104 + (i-6)*32 )*Y_XISHU),  &m_MJPGList, IDC_EDIT_FASTDIALS_NUMBER01);
                m_edtNumber[i].SetLimitText(15);
                m_edtNumber[i].SetLimitDiagital();
            }
        }
    ...
    void CFastDialsDlg::SetFastDialParam(Util::shared_ptr<Data::Setting> data)
    {
        m_Data = data;
