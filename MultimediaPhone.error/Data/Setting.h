...
namespace Data {
    ...
    public:
        ...
        */
        ...
        void sysVolume(int const sysVolume)
        {
            sysVolume_ = sysVolume;
        }
		int const sysVolume() const
        {
            return sysVolume_;
        }
		...
		//blacklight
		int const blackLightValue() const
		{
			return blackLightValue_;
		}
		void blackLightValue(int const blackLightValue)
		{
			blackLightValue_ = blackLightValue;
		}
		bool const isContrlBlackLight() const
		{
			return isContrlBlackLight_;
		}
		void isContrlBlackLight(bool const isContrlBlackLight)
		{
			isContrlBlackLight_ = isContrlBlackLight;
		}

		int const contrlBlackLightWaitTime() const
		{
			return contrlBlackLightWaitTime_;
		}
		void contrlBlackLightWaitTime(int const contrlBlackLightWaitTime)
		{
			contrlBlackLightWaitTime_ = contrlBlackLightWaitTime;
		}
		bool const isNightControlBlackLight() const
		{
			return isNightControlBlackLight_;
		}
		void isNightControlBlackLight(bool const isNightControlBlackLight)
		{
	...
	public:
		Setting(); //create
		//~Setting(); //destroy
        static std::vector<Util::shared_ptr<Setting> > GetFromDatabase(std::string const& filter, std::string const& orderFieldName = "id", SeqenceRelation const dir = srNull, int const offset = 0, int const pageSize = -1); //get from database
		static Util::shared_ptr<Setting> GetCurrentConfig();
		static Util::shared_ptr<Setting> GetDefaultConfig();
		static Util::shared_ptr<Setting> GetNormalConfig();
		static Util::shared_ptr<Setting> GetSpeedConfig();
		static Util::shared_ptr<Setting> GetSpaceConfig();
		void Update() const; //sync to database
		void Insert(); //insert new instance to database
		static void Remove(std::string filter); //delete form database
	private:
		static void modifyFieldByDB_(int argc, char** argv, char** columnName, Util::shared_ptr<Setting> item);
	};
}
