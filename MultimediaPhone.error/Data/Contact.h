...
namespace Data {
    ...
    public:
        ...
        */
        ...
        std::vector<Util::shared_ptr<ContactInfo> > histories();
    private:
        //int id_;
        bool groupFinded_;
        ...
        std::vector<Util::shared_ptr<ContactInfo> > histories_;
    public:
        Contact(); //create
        //~Contact(); //destroy
        static std::vector<Util::shared_ptr<Contact> > GetFromDatabase(std::string const& filter, std::string const& orderFieldName = "id", SeqenceRelation const sr = srDesc, int const offset = 0, int const pageSize = -1);
