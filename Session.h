...
#include "../Util/rsaeuro/source/rsaeuro.h"
...
namespace Protocol {
    ...
    protected:
        Util::ResponseMessage const auth_(std::map<std::string, std::string>& values, std::string const& digestUriValue) const;
        Util::ResponseMessage const timeout_() const;
        ...
        std::string const rsaEncript_(R_RSA_PUBLIC_KEY const* const platformPublicKey, std::string const& deviceId, std::string const& randomKey);
    protected:
        Util::Communicator* communicator_;
        static int nc_;
