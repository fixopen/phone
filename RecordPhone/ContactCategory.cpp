...
namespace Util {
    std::wstring DataAccess<ContactCategory>::tableName_ = L"contactCategory";
    int DataAccess<ContactCategory>::count_ = 0;
	int DataAccess<ContactCategory>::offset_ = 0;
}

std::vector<Util::shared_ptr<Contact> > const ContactCategory::contacts() const {
    if (!contactsFinded_) {
        contacts_ = Contact::Select(L"[categoryId] = " + Util::StringOp::FromInt(id()));
...
void ContactCategory::Update() const {
    ...
    cmd += Util::StringOp::FromInt(useDefaultRingtone_);
	cmd += L", [ringtoneName] = '";
	cmd += ringtoneName_;
    cmd += L"', [useDefaultPrompt] = ";
    cmd += Util::StringOp::FromInt(useDefaultPrompt_);
	cmd += L"', [promptName] = '";
	cmd += promptName_;
    cmd += L"' WHERE [id] = ";
...
void ContactCategory::modifyFieldByDB_(int argc, char** argv, char** columnName, Util::shared_ptr<ContactCategory>& item) {
    ...
    item->ringtoneName_ = Util::StringOp::FromUTF8(argv[GetIndexByName(argc, columnName, "ringtoneName")]);
    item->useDefaultPrompt_ = !!atoi(argv[GetIndexByName(argc, columnName, "useDefaultPrompt")]);
    item->promptName_ = Util::StringOp::FromUTF8(argv[GetIndexByName(argc, columnName, "promptName")]);
}
