...
//	CPaintDC dc(this); // device context for painting
	...
	if(m_sDate1 != "")
	{
		...
		if(m_sDate2.GetLength() == 3)
		{
			...
			pdc->DrawText(s1, &rt, DT_WORDBREAK | DT_CENTER);
		}
		else
		{
		...
		pdc->DrawText(s2, &rt, DT_WORDBREAK | DT_CENTER);
		...
		if(m_sDate2.GetLength() == 3)
		{
			...
			pdc->DrawText(s3, &rt, DT_WORDBREAK | DT_CENTER);
		}
	}
	else if(s != "")
	{
...
//	pDC.BitBlt (rect.left, rect.top, rect.Width (), rect.Height(), &memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
	for(int n = 0; n < len; n++)
	{
		char c = datetime[n];
		int index = c - '0';
		TransparentBlt2(pDC.m_hDC, rt3.left+18+n*16, rt3.top+5, 14, 23, memDC.m_hDC, index*14, 0, 14, 23,  RGB(255, 0, 255));
	}
...
BOOL CCEMoveTxtStatic::Create(LPCTSTR sTxt, int style, CRect &rect, CWnd *pParentWnd, int nCtrlID)
{
	...
	memDC.FillSolidRect(CRect(0, 0, w, h),   RGB(192,   192,   192));   
...
//	static CPoint g_OldPoint = CPoint(0xFFFF, 0xFFFF);
...
void CCEFrameStatic::OnPaint()
{
	...
	for(int nIndex = 0; nIndex < m_ntxtCount; nIndex++)
	{
		...
		int oldcolor = pdc->SetTextColor(m_pTxtStruct[nIndex].txtColor);
		CString s = m_pTxtStruct[nIndex].sTxt;
		pdc->DrawText(s, &rect1, DT_WORDBREAK |m_pTxtStruct[nIndex].sAlign);
...
void CCEFrameStatic::SetTextStruct(TextStruct *pTxtStruct, int ncount)
{
	m_ntxtCount = ncount;
...
void CCEFrameStatic ::ChangeTextStruct(int nIndex, TextStruct txtStruct)
{
...
void CCEFramePartStatic::OnPaint()
{
	...
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainBackRGB[Data::g_skinstyle]);

	//画左边Title top
	rect1 = CRect(0, 0, 136+4+1, 0+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1TopRGB[0][Data::g_skinstyle]);
	rect1 = CRect(1, 1, 137+4+1, 1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1TopRGB[1][Data::g_skinstyle]);
	rect1 = CRect(2, 2, 138+4+1, 2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1TopRGB[2][Data::g_skinstyle]);
	rect1 = CRect(3, 3, 139+4+1, 3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1TopRGB[3][Data::g_skinstyle]);

	//画左边Title bottom
	rect1 = CRect(9, 25, 159+4+1, 25+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1BottomRGB[0][Data::g_skinstyle]);
	rect1 = CRect(9-1, 25-1, 159+4-1+1, 25-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1BottomRGB[1][Data::g_skinstyle]);
	rect1 = CRect(9-2, 25-2, 159+4-2+1, 25-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1BottomRGB[2][Data::g_skinstyle]);
	rect1 = CRect(9-3, 25-3, 159+4-2+1, 25-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1BottomRGB[3][Data::g_skinstyle]);
	
	//画左边Title left
	rect1 = CRect(0, 0, 0+1, 103+4+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1LeftRGB[0][Data::g_skinstyle]);
	rect1 = CRect(0+1, 0+1, 0+1+1, 103+4-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1LeftRGB[1][Data::g_skinstyle]);
	rect1 = CRect(0+2, 0+2, 0+2+1, 103+4-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1LeftRGB[2][Data::g_skinstyle]);
	rect1 = CRect(0+3, 0+3, 0+3+1, 103+4-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1LeftRGB[3][Data::g_skinstyle]);

	//画左边Title right
	rect1 = CRect(9, 25, 9+1, 98);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1RightRGB[0][Data::g_skinstyle]);
	rect1 = CRect(9-1, 25-1, 9-1+1, 98+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1RightRGB[1][Data::g_skinstyle]);
	rect1 = CRect(9-2, 25-2, 9-2+1, 98+2);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1RightRGB[2][Data::g_skinstyle]);
	rect1 = CRect(9-3, 25-3, 9-3+1, 98+3);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1RightRGB[3][Data::g_skinstyle]);
	...
	for(i = 4; i < 22; i++)
	{
		...
		pdc->FillSolidRect(&rect1, Data::g_partFrameTitle1BackRGB[Data::g_skinstyle]);
		x++;
	}
	x = 100;
	for(i = 4; i < 6; i++)
	...
	CPen *oldpen = pdc->SelectObject(&pen);
	pdc->MoveTo(CPoint(1, 104));
	pdc->LineTo(CPoint(9, 96));
	CPen pen1(PS_SOLID, 1, Data::g_partFrameTitle1XiexianRGB[1][Data::g_skinstyle]);
	pdc->SelectObject(&pen1);
	pdc->MoveTo(CPoint(1, 105));
	pdc->LineTo(CPoint(9, 97));
	CPen pen2(PS_SOLID, 1, Data::g_partFrameTitle1XiexianRGB[2][Data::g_skinstyle]);
	pdc->SelectObject(&pen2);
	pdc->MoveTo(CPoint(1, 106));
	pdc->LineTo(CPoint(10, 97));
	CPen pen3(PS_SOLID, 1, Data::g_partFrameTitle1XiexianRGB[3][Data::g_skinstyle]);
	pdc->SelectObject(&pen3);
	pdc->MoveTo(CPoint(1, 107));
	pdc->LineTo(CPoint(10, 98));
	
	CPen pen4(PS_SOLID, 1, Data::g_partFrameTitle2XiexianRGB[0][Data::g_skinstyle]);
	pdc->SelectObject(&pen4);
	pdc->MoveTo(CPoint(137, 1));
	pdc->LineTo(CPoint(160, 24));
	CPen pen5(PS_SOLID, 1, Data::g_partFrameTitle2XiexianRGB[1][Data::g_skinstyle]);
	pdc->SelectObject(&pen5);
	pdc->MoveTo(CPoint(138, 1));
	pdc->LineTo(CPoint(161, 24));
	CPen pen6(PS_SOLID, 1, Data::g_partFrameTitle2XiexianRGB[2][Data::g_skinstyle]);
	pdc->SelectObject(&pen6);
	pdc->MoveTo(CPoint(139, 1));
	pdc->LineTo(CPoint(162, 24));
	CPen pen7(PS_SOLID, 1, Data::g_partFrameTitle2XiexianRGB[3][Data::g_skinstyle]);
	pdc->SelectObject(&pen7);
	pdc->MoveTo(CPoint(140, 1));
	pdc->LineTo(CPoint(163, 24));
	pdc->SelectObject(oldpen);

	//画右边Title top
	rect1 = CRect(140, 0, 414+1, 0+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2TopRGB[0][Data::g_skinstyle]);
	rect1 = CRect(140+1, 1, 414-1+1, 1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2TopRGB[1][Data::g_skinstyle]);
	rect1 = CRect(140+2, 2, 414-2+1, 2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2TopRGB[2][Data::g_skinstyle]);
	rect1 = CRect(140+3, 3, 414-3+1, 3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2TopRGB[3][Data::g_skinstyle]);

	//画右边Title bottom
	rect1 = CRect(164, 25, 414+1, 25+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2BottomRGB[0][Data::g_skinstyle]);
	rect1 = CRect(164-1, 25-1, 414-1+1, 25-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2BottomRGB[1][Data::g_skinstyle]);
	rect1 = CRect(164-2, 25-2, 414-2+1, 25-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2BottomRGB[2][Data::g_skinstyle]);
	rect1 = CRect(164-3, 25-3, 414-3+1, 25-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2BottomRGB[3][Data::g_skinstyle]);
	
	//画右边Title right
	rect1 = CRect(414, 0, 414+1, 25+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2RightRGB[0][Data::g_skinstyle]);
	rect1 = CRect(414-1, 1, 414-1+1, 25-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2RightRGB[1][Data::g_skinstyle]);
	rect1 = CRect(414-2, 2, 414-2+1, 25-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2RightRGB[2][Data::g_skinstyle]);
	rect1 = CRect(414-3, 3, 414-3+1, 25-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2RightRGB[3][Data::g_skinstyle]);
	...
	for(i = 4; i < 22; i++)
	{
		...
		pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2BackRGB[Data::g_skinstyle]);
	}
	//画线
	rect1 = CRect(150, 10, 387+1, 10+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2LineRGB[0][Data::g_skinstyle]);
	rect1 = CRect(151, 11, 387+1, 11+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameTitle2LineRGB[1][Data::g_skinstyle]);
	...
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainLeftRGB[0][Data::g_skinstyle]);
	rect1 = CRect(0+1, 108-1, 0+1+1, 143-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainLeftRGB[1][Data::g_skinstyle]);
	rect1 = CRect(0+2, 108-2, 0+2+1, 143-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainLeftRGB[2][Data::g_skinstyle]);
	rect1 = CRect(0+3, 108-3, 0+3+1, 143-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainLeftRGB[3][Data::g_skinstyle]);
	
	//bottom
	rect1 = CRect(0, 143, 414+1, 143+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainBottomRGB[0][Data::g_skinstyle]);
	rect1 = CRect(0+1, 143-1, 414-1+1, 143-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainBottomRGB[1][Data::g_skinstyle]);
	rect1 = CRect(0+2, 143-2, 414-2+1, 143-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainBottomRGB[2][Data::g_skinstyle]);
	rect1 = CRect(0+3, 143-3, 414-3+1, 143-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainBottomRGB[3][Data::g_skinstyle]);
	rect1 = CRect(0+4, 143-4, 414-4+1, 143-4+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainBottomRGB[4][Data::g_skinstyle]);
	//right
	rect1 = CRect(414, 26, 414+1, 143+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainRightRGB[0][Data::g_skinstyle]);
	rect1 = CRect(414-1, 26, 414-1+1, 143-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainRightRGB[1][Data::g_skinstyle]);
	rect1 = CRect(414-2, 26, 414-2+1, 143-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainRightRGB[2][Data::g_skinstyle]);
	rect1 = CRect(414-3, 26, 414-3+1, 143-3+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainRightRGB[3][Data::g_skinstyle]);
	rect1 = CRect(414-4, 26, 414-4+1, 143-4+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameMainRightRGB[4][Data::g_skinstyle]);

	//画InRect
	//top
	rect1 = CRect(15, 31, 333+1, 31+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectTopRGB[0][Data::g_skinstyle]);
	rect1 = CRect(15-1, 31-1, 333+1+1, 31-1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectTopRGB[1][Data::g_skinstyle]);
	rect1 = CRect(15-2, 31-2, 333+2+1, 31-2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectTopRGB[2][Data::g_skinstyle]);
	rect1 = CRect(11, 27, 409+1, 27+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectTopRGB[3][Data::g_skinstyle]);

	//bottom
	rect1 = CRect(15, 134, 333+1, 134+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectBottomRGB[0][Data::g_skinstyle]);
	rect1 = CRect(15-1, 134+1, 333+1+1, 134+1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectBottomRGB[1][Data::g_skinstyle]);
	rect1 = CRect(15-2, 134+2, 333+2+1, 134+2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectBottomRGB[2][Data::g_skinstyle]);

	//left
	rect1 = CRect(15, 32,15+1, 133+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectLeftRGB[0][Data::g_skinstyle]);
	rect1 = CRect(15-1, 32-1, 15-1+1, 133+1+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectLeftRGB[1][Data::g_skinstyle]);
	rect1 = CRect(15-2, 32-2, 15-2+1, 133+2+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectLeftRGB[2][Data::g_skinstyle]);
	rect1 = CRect(11, 28, 11+1, 98+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectLeftRGB[3][Data::g_skinstyle]);

	//right
	rect1 = CRect(329, 32, 329+1, 133+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectRightRGB[0][Data::g_skinstyle]);
	rect1 = CRect(329+1, 32, 329+1+1, 133+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectRightRGB[1][Data::g_skinstyle]);
	rect1 = CRect(329+2, 32, 329+2+1, 133+1);
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectRightRGB[2][Data::g_skinstyle]);
	...
	pdc->FillSolidRect(&rect1, Data::g_partFrameInRectBackRGB[Data::g_skinstyle]);
...
void CCEFramePartStatic::SetTextStruct(TextStruct *pTxtStruct, int ncount)
{
	m_ntxtCount = ncount;
	m_pTxtStruct  = new TextStruct[ncount];
	memset(m_pTxtStruct, 0, sizeof(TextStruct) * m_ntxtCount);
...
void CCEFramePartStatic ::ChangeTextStruct(int nIndex, TextStruct txtStruct)
{
	if(nIndex < m_ntxtCount)
	{
		memcpy(&m_pTxtStruct[nIndex], &txtStruct, sizeof(TextStruct));
