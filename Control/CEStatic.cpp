...
void CCEStatic::PreSubclassWindow() {
    ...
    // Create the font
    VERIFY(m_Font.CreateFont(m_nFontHeight,                        // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
...
void CCEStatic::OnDraw(CRect rt_, CString s) {
    HDC dc = ::GetDC(m_hWnd);
    ...
    if (m_nBitmapID > 0) {
        ...
        CRect rect;
        GetClientRect(&rect);
        cb = ::LoadBitmap(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(m_nBitmapID));//载入资源
    ...
    if (m_sDate1 != "") {
        ...
        ::DrawText(dc, m_sDate1, m_sDate1.GetLength(), &rt, DT_WORDBREAK | DT_CENTER);
        ...
        rt.left = size;
        rt.right = size + size1;
        //  dc.FillSolidRect(rt, Data::g_listctrlBackRGB2[Data::g_skinstyle]);
        if (m_sDate2.GetLength() == 3) {
            CString s1 = m_sDate2.Mid(1, 1);
            ::DrawText(dc, s1, s1.GetLength(), &rt, DT_WORDBREAK | DT_LEFT);
        } else {
        ...
        if (m_sDate2.GetLength() == 3) {
            ...
            //      dc.FillSolidRect(rt, Data::g_listctrlBackRGB2[Data::g_skinstyle]);
            ::DrawText(dc, s3, s3.GetLength(), &rt, DT_WORDBREAK | DT_LEFT);
        }
    } else if (s != "") {
    ...
    if (m_bIsUnderLine) {
        ...
        HPEN pOldPen = (HPEN)::SelectObject(dc, pen.m_hObject);
        POINT pt;
        ::MoveToEx(dc, rt.left, rt.bottom - 1, &pt);
        ::LineTo(dc, rt.right, rt.bottom - 1);
    ...
    if (m_bIsBorder) {
        ...
        dc.FillSolidRect(&rect1, Data::g_staticLine1RGB[0][Data::g_skinstyle]);
        rect1 = CRect(rrt.left, rrt.bottom-1, rrt.right+1, rrt.bottom);
        dc.FillSolidRect(&rect1, Data::g_staticLine1RGB[1][Data::g_skinstyle]);
        ...
        rrt.DeflateRect(1, 1);
        rect1 = CRect(rrt.left, rrt.top, rrt.right+1, rrt.top+1);
        dc.FillSolidRect(&rect1, Data::g_staticLine2RGB[0][Data::g_skinstyle]);
        rect1 = CRect(rrt.left, rrt.bottom-1, rrt.right+1, rrt.bottom);
        dc.FillSolidRect(&rect1, Data::g_staticLine2RGB[1][Data::g_skinstyle]);
        rect1 = CRect(rrt.left, rrt.top, rrt.left+1, rrt.bottom+1);
        dc.FillSolidRect(&rect1, Data::g_staticLine2RGB[0][Data::g_skinstyle]);
        rect1 = CRect(rrt.right, rrt.top, rrt.right+1, rrt.bottom+1);
        dc.FillSolidRect(&rect1, Data::g_staticLine2RGB[1][Data::g_skinstyle]);
        rrt.DeflateRect(1, 1);
        rect1 = CRect(rrt.left, rrt.top, rrt.right+1, rrt.top+1);
        dc.FillSolidRect(&rect1, Data::g_staticLine3RGB[0][Data::g_skinstyle]);
        rect1 = CRect(rrt.left, rrt.bottom-1, rrt.right+1, rrt.bottom);
        dc.FillSolidRect(&rect1, Data::g_staticLine3RGB[1][Data::g_skinstyle]);
        rect1 = CRect(rrt.left, rrt.top, rrt.left+1, rrt.bottom+1);
        dc.FillSolidRect(&rect1, Data::g_staticLine3RGB[0][Data::g_skinstyle]);
        rect1 = CRect(rrt.right, rrt.top, rrt.right+1, rrt.bottom+1);
        dc.FillSolidRect(&rect1, Data::g_staticLine3RGB[1][Data::g_skinstyle]);
...
void CCEStatic::OnPaint() {
    ...
    if(m_bIsToggle && m_toggleRGB != RGB(0, 0, 0))
    {
    dc.FillSolidRect(&rt, m_toggleRGB);
    }
    else
    ...
    dc.DrawText(m_sDate1, &rt, DT_WORDBREAK | DT_CENTER);
    int size1 = 14;  //14
    SetFontSize(size1);
    dc.SelectObject(&m_Font);
    rt.top=0;
    ...
    {
    CString s1 = m_sDate2.Mid(1, 1);
    dc.DrawText(s1, &rt, DT_WORDBREAK | DT_LEFT);
    }
    else
    {
    CString s1 = m_sDate2.Left(1);
    ...
    CString s2 = m_sDate2.Right(1);
    //  dc.FillSolidRect(rt, Data::g_listctrlBackRGB2[Data::g_skinstyle]);
    dc.DrawText(s2, &rt, DT_WORDBREAK | DT_LEFT);
    if(m_sDate2.GetLength() == 3)
    {
    rt.left = 2;
    rt.right = size-1;      
    ...
    CString s3 = m_sDate2.Left(1);
    //      dc.FillSolidRect(rt, Data::g_listctrlBackRGB2[Data::g_skinstyle]);
    dc.DrawText(s3, &rt, DT_WORDBREAK | DT_LEFT);
    }
    }
    else if(s != "")
    {
    ...
    dc.DrawText(s, &rt, DT_WORDBREAK | m_nAlign);
    }
    if(m_bIsUnderLine)
    {       
    CPen pen (PS_SOLID, 1, m_txtRGB);
    ...
    dc.LineTo(rt.right, rt.bottom-1);
...
void CCETimeStatic::DrawItem(LPDRAWITEMSTRUCT lpDIS) {
    ...
    sprintf_s(datetime, 64, "%04d%s%02d%s%02d%s %s", curtime.wYear, Data::LanguageResource::Get(Data::RI_COMN_YEARSTC).c_str(), curtime.wMonth, Data::LanguageResource::Get(Data::RI_COMN_MONTHSTC).c_str(), curtime.wDay, Data::LanguageResource::Get(Data::RI_COMN_DAYSTC).c_str(), Data::LanguageResource::Get(Data::RI_COMN_SUNSTC + nWeekDay).c_str());
    str1 = datetime;
    ...
    if (m_bChineseVersion)  //if chinese language
    {
        ...
        if (nLeapMonth == nM) {
            strcpy_s(chLeapMonth, 16, "润");
        }
        sprintf_s(datetime, 64, "%s %s%s %s %s%s %s", Data::LanguageResource::Get(Data::RI_MAIN_LUNARCALENDARSTC).c_str(), chYear, Data::LanguageResource::Get(Data::RI_COMN_YEARSTC).c_str(), chLeapMonth, chMonth, chDay, chHoliday);
        str1 = datetime;
    ...
    sprintf_s(datetime, 64, "%02d:%02d:%02d", curtime.wHour, curtime.wMinute, curtime.wSecond);
...
void CCEMoveTxtStatic::SetTxt(LPCTSTR sTxt, int height, COLORREF rgb) {
    m_font = new CFont();
    m_font->CreateFont(height,              // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
...
void CCEFrameStatic::OnDrawRect() {
    ...
    for (int nIndex = 0; nIndex < m_ntxtCount; nIndex++) {
        VERIFY(m_Font.CreateFont(m_pTxtStruct[nIndex].txtFontSize,                        // nHeight
            0,                         // nWidth
            0,                         // nEscapement
            0,                         // nOrientation
            FW_NORMAL,                 // nWeight
