...
public:
    ...
    void SetTogle(BOOL isToggle, BOOL flag = FALSE) {
        m_bIsToggle = isToggle; if (flag) {
                                    OnPaint();
                                }
    }
    //设置左边开始空几个象素开始显示文字
    void SetLeftMargin(int nPixel) {
        m_nLeftMargin = nPixel;
    }
    //设置字体的大小
    ...
    //设置文字对齐方式
    void SetAlign(int nAglin) {
        m_nAlign = nAglin;
    }
    //设置是否border
    void SetBorder(BOOL isBorder) {
        m_bIsBorder = isBorder;
    }
    //设置toogle背景色
    void SetToggleColor(COLORREF cl) {
        m_toggleRGB = cl;
    }

    //用于日历控件
    CString m_sDate1, m_sDate2;
    void SetDateTxt(CString sDate1, CString sDate2) {
        m_sDate1 = sDate1; m_sDate2 = sDate2;
    }
    void GetDateTxt(CString& s) {
        s = m_sDate1;
    }
...
public:
    ...
    void SetInRect(CRect rect, int nInRect = 0) {
        m_InRect = rect; m_nInRectWidth = nInRect;
    }
    void OnDrawRect();
    CCEFrameStatic();
