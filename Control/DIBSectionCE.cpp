...
RGBQUAD CDIBSectionCE::ms_StdColours[] = {
    ...
    { 0xFF, 0xFF, 0xFF, 0 }, { 0x00, 0x00, 0xFF, 0 },    // First 16 will be palette for 16 colour bitmaps
    { 0x00, 0xFF, 0x00, 0 }, { 0x00, 0xFF, 0xFF, 0 }, { 0xFF, 0x00, 0x00, 0 }, { 0xFF, 0x00, 0xFF, 0 }, { 0xFF, 0xFF, 0x00, 0 }, { 0x00, 0x00, 0x80, 0 }, { 0x00, 0x80, 0x00, 0 }, { 0x00, 0x80, 0x80, 0 }, { 0x80, 0x00, 0x00, 0 }, { 0x80, 0x00, 0x80, 0 }, { 0x80, 0x80, 0x00, 0 }, { 0x80, 0x80, 0x80, 0 }, { 0xC0, 0xC0, 0xC0, 0 },
};
...
/*static*/ int CDIBSectionCE::NumColorEntries(int nBitsPerPixel) {
               ...
               case 24:
               case 32:
                   nColors = 0;   break;   // 16,24 or 32 bpp have no color table
...
BOOL CDIBSectionCE::Draw(CDC* pDC, CPoint ptDest, BOOL bForceBackground /*=FALSE*/) {
    ...
    BOOL bResult = pDC->BitBlt(ptDest.x, ptDest.y, size.cx, size.cy, pMemDC, ptOrigin.x, ptOrigin.y, SRCCOPY);
...
BOOL CDIBSectionCE::Stretch(CDC* pDC, CPoint ptDest, CSize size, BOOL bForceBackground /*=FALSE*/) {
    if (!m_hBitmap) {
        return FALSE;
    }
    ...
    BOOL bResult = pDC->StretchBlt(ptDest.x, ptDest.y, size.cx, size.cy, pMemDC, ptOrigin.x, ptOrigin.y, imagesize.cx, imagesize.cy, SRCCOPY);
...
BOOL CDIBSectionCE::SetBitmap(LPCTSTR lpszResourceName) {
    HBITMAP hBmp = (HBITMAP)::LoadImage(AfxGetResourceHandle(), lpszResourceName, IMAGE_BITMAP, 0, 0,
#ifdef _WIN32_WCE
        0
#else
        LR_CREATEDIBSECTION
...
BOOL CDIBSectionCE::SetBitmap(LPBITMAPINFO lpBitmapInfo, LPVOID lpBits) {
    ...
    TRY
    {
        ...
        if (dwImageSize == 0) {
            int nBytesPerLine = BytesPerLine(lpBitmapInfo->bmiHeader.biWidth, lpBitmapInfo->bmiHeader.biBitCount);
...
BOOL CDIBSectionCE::SetBitmap(HBITMAP hBitmap, CPalette* pPalette /*= NULL*/) {
    ...
    TRY
    {
        ...
        // Create it!
        m_hBitmap = CreateDIBSection(dc.m_hDC, (const BITMAPINFO *)m_DIBinfo, m_iColorDataType, &m_ppvBits, NULL, 0);
#ifndef DIBSECTION_NO_PALETTE
        if (pOldPalette) {
