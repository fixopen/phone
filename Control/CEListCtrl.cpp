...
BOOL CCEListCtrl ::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) {
    BOOL result = FALSE;
    //类型安全转换   
...
BOOL CCEListCtrl ::Create(DWORD dwStyle, CRect& rect, CWnd* pParentWnd, UINT nID, BOOL bIsScroll, int Scroll_style, BOOL isSelfScroll) {
    ...
    VERIFY(m_font.CreateFont(m_nLineHeight,                        // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
    ...
    if (m_bIsScroll) {
        m_nPage = rect.Height() / 32;/*m_nLineHeight*/
        CRect rt ;
        GetWindowRect(&rt);
...
void CCEListCtrl::OnClick(NMHDR* pNMHDR, LRESULT* pResult) {
    ...
    int nSubSel = pListView->iSubItem; 
    int nSel = pListView->iItem; 

    GetParent()->PostMessage(WM_LISTCTRL_CLICKSUBITEM, nSel, nSubSel);
