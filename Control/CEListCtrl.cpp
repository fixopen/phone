...
namespace Control {
    ...
    */
    ...
    void CCEListCtrl::OnTimer(UINT nIDEvent) 
    {
        ...
        {
            KillTimer(1);
            GetParent()->PostMessage(WM_LISTCTRL_CLICK, GetDlgCtrlID(), 0); //WM_LISTCTRL_CLICK
        }
            ...
            if(wParam == SB_THUMBPOSITION)
            {
                ...
                if(nPos < lParam)
                {
                    ...
                    for(int i = 0; i < nPage; i++)
                        SendMessage(WM_VSCROLL, SB_PAGEDOWN, 1 );
                    for(int i = 0; i < nY; i++)
                        SendMessage(WM_VSCROLL, SB_LINEDOWN, 1 );
                }
                else
                {
                    ...
                    for(int i = 0; i < nPage; i++)
                        SendMessage(WM_VSCROLL, SB_PAGEUP, 1 );
                    for(int i = 0; i < nY; i++)
                        SendMessage(WM_VSCROLL, SB_LINEUP, 1 );
                }
            }
        }
        return ret;
