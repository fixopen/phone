...
{
	m_RGB1 = RGB(255, 255, 128);
	m_RGB2 = RGB(128, 255, 255);
	m_nLineHeight = 20;
	m_nPage = 0;
}
...
void CCEListCtrl::SetScrollRagle(BOOL redraw)
{
	...
	if(redraw)
	{
		//m_scollbar_.SendMessage(WM_PAINT);
		m_scollbar_.Invalidate();
	}
}

BOOL CCEListCtrl ::Create(DWORD dwStyle, CRect &rect, CWnd *pParentWnd, UINT nID, BOOL bIsScroll, int Scroll_style)
{
