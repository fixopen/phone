...
public:
    ...
    BOOL Create(int         iconNO, //软键盘的图标ID
        ...
        CRect& rect, CWnd* pParentWnd, //父窗口指针
        int style, //风格
...
public:
    ...
    UINT32 m_BitmapID;
    COLORREF m_clrBack;
    void SetBackRGB(COLORREF rgb) {
        m_clrBack = rgb;
    }
    void SetKeyText(CString s, int nkey, COLORREF cr);
    void SetLeftTimer();
    void SetEnableKey(int nKey, BOOL flag) {
        m_pKeys[nKey].bEnable = flag;
    };
...
public:
    ...
    INT32 GetPenKey(CPoint& pt);
    INT8 GetToggleKey();
    void SetToggleKey(int nIndex) {
        m_nToggle = nIndex; Invalidate();
    }
