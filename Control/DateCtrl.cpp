...
BOOL CDateCtrl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, UINT uFontHeight, em_mode mode) {
    ...
    if (bRes) {
        ...
        if (!m_btnDown.Create(_T(""), IDB_BITMAP_RIGHTARROW, IDB_BITMAP_RIGHTARROWD, WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, rcButton, this, ID_BUTTON_DOWN)) {
            return FALSE;
        }
        rcButton.OffsetRect((0 - BUTTON_WIDTH), 0);

        if (!m_btnUp.Create(_T(""), IDB_BITMAP_LEFTARROW, IDB_BITMAP_LEFTARROWD, WS_CHILD | BS_PUSHBUTTON, rcButton, this, ID_BUTTON_UP)) {
            return FALSE;
        }

        m_font.CreateFont(uFontHeight,              // nHeight
            0,                         // nWidth
            0,                         // nEscapement
            0,                         // nOrientation
            FW_NORMAL,                 // nWeight
...
void CDateCtrl::SetTime(const int nYear, const int nMonth, const int nDay, const int nHour, const int nMinute, const int nSecond) {
    ...
    if (nRealYear < RANGE_YEAR_MIN) {
        nRealYear = RANGE_YEAR_MIN;
    }
