...
void CCEDialog::ShowWindow_(int nCmdShow) {
    ...
    if (nCmdShow == SW_HIDE) {
        SipShowIM(SIPF_OFF);
        //      ((CMultimediaPhoneDlg*)(theApp.m_pMainWnd))->m_pMainDlg->m_mainPhotoDlg_->SendMessage(WM_OUTEVENT, 0, 1);
    }
}
...
C3GTitleDialog::C3GTitleDialog(UINT nIDTemplate, CWnd* pParent /*=NULL*/)
: CDialog(nIDTemplate, pParent) {
    //{{AFX_DATA_INIT(CCEDialog)
    // NOTE: the ClassWizard will add member initialization here
    //}}AFX_DATA_INIT
...
void C3GTitleDialog::DoDataExchange(CDataExchange* pDX) {
    CDialog::DoDataExchange(pDX);
    //{{AFX_DATA_MAP(CCEDialog)
    // NOTE: the ClassWizard will add DDX and DDV calls here
    //}}AFX_DATA_MAP
...
BOOL C3GTitleDialog::OnEraseBkgnd(CDC* pDC) {
    // TODO: Add your message handler code here and/or call default
    return TRUE;
    //  return CDialog::OnEraseBkgnd(pDC);
}

BOOL C3GTitleDialog::OnInitDialog() {
    CDialog::OnInitDialog();
    m_3GTitleMJPG.Create(L"", WS_VISIBLE | WS_CHILD, CRect(610, 0, 610 + 190, 70), this);
    m_3GTitleMJPG.SetCurrentLinkFile(".\\adv\\mjpg\\k1\\中文\\3g_telstatus.xml");
    m_3GTitleMJPG.SetMJPGRect(CRect(610, 0, 610 + 190, 70));

    SYSTEMTIME curtime;
    GetLocalTime(&curtime);
    char txt[24];
    sprintf_s(txt, 24, "%04d-%02d-%02d", curtime.wYear, curtime.wMonth, curtime.wDay);
    CString s = txt; 
    m_3GTitleMJPG.SetUnitText(2, s, FALSE);
    sprintf_s(txt, 24, "%02d:%02d", curtime.wHour, curtime.wMinute);
    s = txt;
    m_3GTitleMJPG.SetUnitText(3, s, FALSE);

    SetTimer(999, 1000, NULL);
    return TRUE;
}

void C3GTitleDialog::OnTimer(UINT nIDEvent) {
    if (nIDEvent == 999) {
        if (IsWindowVisible()) {
            SYSTEMTIME curtime;
            GetLocalTime(&curtime);
...
BEGIN_MESSAGE_MAP(C3GTitleDialog, CDialog)
    //{{AFX_MSG_MAP(CCEDialog)
    ON_WM_TIMER()
    ON_WM_ERASEBKGND()
    //}}AFX_MSG_MAP
