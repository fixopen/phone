...
BOOL CCEButtonST::Create(LPCTSTR lpszCaption, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID) {
...
BOOL CCEButtonST::OnClicked() {
    if (m_bIsCheckBox) {
        ...
        CWnd* pWnd = GetParent();
        if (pWnd) {
            pWnd->SendMessage(WM_BTNST_CLICK, GetDlgCtrlID());
        }
    } // if
    else {
...
DWORD CCEButtonST::SetURL(LPCTSTR lpszURL) {
    ...
    if (lpszURL) {
        // Store the URL
        wcsncpy_s(m_szURL, 260, lpszURL, _MAX_PATH);
    } // if
...
DWORD CCEButtonST::OnDrawBorder(CDC* pDC, LPCRECT pRect) {
    ...
    } else if (m_bIsFlat == FALSE || (m_bIsFlat && m_bMouseOnButton)) {
...
CCEBmpButton::CCEBmpButton() {
    m_nStatus = 0;    // 0 up 1 down
    m_bToggle = FALSE;
...
BOOL CCEBmpButton::Create(LPCTSTR lpszCaption, int nIDUp, int nIDDown, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID) {
    m_nBitmapUp = nIDUp;
...
void CCEBmpButton::OnLButtonDown(UINT nFlags, CPoint point) {
    if (!m_bToggle) {
        m_nStatus = 1;
    }
    Invalidate();
    ...
    //  if(GetParent())
    //      GetParent()->SendMessage(UM_BTN_DOWN, nID, nFlags);
    CButton::OnLButtonDown(nFlags, point);
}

void CCEBmpButton::OnLButtonUp(UINT nFlags, CPoint point) {
    if (!m_bToggle) {
        m_nStatus = 0;
    }
    Invalidate();
    ...
    //  int nID = GetDlgCtrlID();
    //  if(GetParent())
    //      GetParent()->SendMessage(UM_BTN_UP, nID, nFlags);
    CButton::OnLButtonUp(nFlags, point);
}

void CCEBmpButton::OnMouseMove(UINT nFlags, CPoint point) {
    //CButton::OnLButtonUp(nFlags, point);
...
void CCEBmpButton::OnTimer(UINT nIDEvent) {
    if (nIDEvent == 1) {
        int nID = GetDlgCtrlID();
        if (GetParent()) {
            GetParent()->SendMessage(UM_BTN_UP, nID, 0);
        }
    }
    CButton::OnTimer(nIDEvent);
}

BEGIN_MESSAGE_MAP(CCEBmpButton, CButton)
    //{{AFX_MSG_MAP(CCEBmpButton)
    //  ON_WM_PAINT()
    ON_WM_LBUTTONUP()
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
...
/////////////////////////////////////////////////////////////////////////////
// CCEBmpButton message handlers

void CCEBmpButton::DrawItem(LPDRAWITEMSTRUCT lpDIS) {
    CDC* pdc = CDC::FromHandle(lpDIS->hDC);

    if (m_nBitmapDown > 0 && m_nBitmapUp > 0) {
        int nID;
        if (!m_bToggle) {
            if (m_nStatus == 0) //up
            {
        ...
        TransparentBlt2(pdc->m_hDC, rect.left, rect.top, rect.Width(), rect.Height(), memDC.m_hDC, m_nBmpOffx, m_nBmpOffy, rect.Width(), rect.Height(), RGB(0xFF, 0, 0xFF));
        ...
        pdc->DrawText(s, s.GetLength(), rect, DT_WORDBREAK | DT_CENTER);
        //      ReleaseDC(pdc);
    }
}

/*
void CCEBmpButton::OnPaint() 
{
...
*/
void CCEBmpButton::PreSubclassWindow() {
...
void CCEBmpButton::SetBmpID(int nIDUp, int nIDDown, BOOL redraw) {
    m_nBitmapUp = nIDUp; 
    m_nBitmapDown = nIDDown;
    if (redraw) {
        Invalidate();
