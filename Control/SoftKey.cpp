...
void CSoftKey::OnLButtonDown(UINT nFlags, CPoint point) {
    ...
    if (m_nCurSel >= 0) {
        ::InvertRect(pdc, m_pKeys[m_nCurSel].sKeyRect);
        m_nCurSel = -1;
    }
    if (sel >= 0) {
        SetTimer(4, 2000, NULL);
        ::InvertRect(pdc, m_pKeys[sel].sKeyRect);
        m_nCurSel = sel;
        m_nLocal = m_nCurSel;
    }
...
BOOL CSoftKey::Create(int       iconNO, CPoint  point, UINT32      nKeyCount, CRect* pKeyRectArray, UINT16* pKeyValueArray, CRect& rect, CWnd* pParent, int style, UINT32      nCtrlId) {
    //2005.5.8
    m_BitmapID = iconNO;
    m_pPoint = point;
    m_nLocal = 0;
    ...
    VERIFY(m_font.CreateFont(22,                        // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
...
void HwWritePrc(void) {
    //typedef int (*HWRecognizePtr)(const THWPoint* pTrace, const THWAttribute* pAttr, unsigned short* pResult);
    //HINSTANCE lib = ::LoadLibrary(L"HWRes.dll");
    //HWRecognizePtr recognize = (HWRecognizePtr)::GetProcAddress(lib, L"HWRecognize");
    ...
    while (TRUE) {
        if (m_nRecognizeBuff != m_nWriteLineBuff) {
            ...
            //Result;
            int iCodeNum = 0;//recognize((const THWPoint *)(m_pWriteLineBuff+m_nRecognizeBuff), &attr, Result); //HWRecognize((const THWPoint *)(m_pWriteLineBuff+m_nRecognizeBuff), &attr, Result);
            m_nRecognizeBuff++;
...
void AddWriteLineBuff(CPoint pt) {
    if (m_pWriteLineBuff) {
        if (m_nWriteLinePoint < WRITE_LINE_POINTMAX) {
            m_pWriteLineBuff[m_nWriteLineBuff].pt[m_nWriteLinePoint].x = (short)pt.x;
            m_pWriteLineBuff[m_nWriteLineBuff].pt[m_nWriteLinePoint].y = (short)pt.y;
...
void CCEIconSoftKey::OnPaint() {
    CPaintDC dc(this); // device context for painting
    CFont m_Font;
    VERIFY(m_Font.CreateFont(16,                        // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
...
BOOL CCEIconSoftKey::Create(int     iconNO, int         toggleIconNo, int           nToggleKey, CPoint      point, UINT32      nKeyCount, ICONKEYSTRUCT* softkeyval, CRect& rect, CWnd* pParent, int style, UINT32      nCtrlId) {
    //2005.5.8
    m_BitmapID = iconNO;
    m_pPoint = point;
...
void CCEIconSoftKey::OnTimer(UINT nIDEvent) {
    if (nIDEvent == 10) {
        ...
        sprintf_s(time, 24, "%02d:%02d", curtime.wHour, curtime.wMinute);
        s = time;
...
#define WRITE_INPUT_TABLE   "/NandFlash/Resource/res/write.dic"
int HWRE_LoadDict(unsigned char** hDic) {
    FILE* in = 0;
    long nLen;
    unsigned char* pDic;

    fopen_s(&in, WRITE_INPUT_TABLE, "rb");
    if (in == NULL) {
        return -1;
    }
