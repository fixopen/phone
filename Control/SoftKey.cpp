...
#include "../inc/jAssociativeWord.h"
#include "../inc/HZRecog.h"
...
void CSoftKey::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if(m_nCurSel >= 0)
    {
		...
		this->GetParent()->SendMessage(WM_SOFTKEY,\
			UINT32(m_pKeys[m_nCurSel].nKeyValue), GetDlgCtrlID());
	    m_nCurSel = -1;
    }
...
void HwWritePrc(void)
{
	...
	while(TRUE)
	{
		...
		{
			//int iCodeNum = HWRecognize((const THWPoint *)(m_pWriteLineBuff+m_nRecognizeBuff), &attr, Result);
			int iCodeNum = HZCharacterRecognize((short *)(m_pWriteLineBuff+m_nRecognizeBuff), Result);
			m_nRecognizeBuff++;
			...
			{	
				...
				//GetInputDlg()->SendMessage(WM_SOFTKEY_REG, \
					(WPARAM)Result, (LPARAM)iCodeNum );
				
				//new
				theApp.handWriteDlg_->SendMessage(WM_SOFTKEY_REG,\
					(WPARAM)Result, (LPARAM)iCodeNum );
			}
		}
		else
...
void CCEIconSoftKey::OnPaint() 
{
	...
	//	dc.BitBlt(x, y, n, rect.Height(), &memDC, TOGGLEBMP_WIDTH-n+1, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
		dc.BitBlt(x, y, n, rect.Height(), &memDC, x, y, SRCCOPY);//将内存场景的图象拷贝到屏幕上
		memDC.SelectObject (oldbitmap);
		cb.DeleteObject();
	}
	...
	for(int i = 0; i < m_nKeyCount; i++)
	{
		...
		if(m_pKeys[i].nBitmapId > 0)
		{
			...
			{
				//居中 by qi
				rectBmp.top    += (rect.Height()-ICON_PIXEL)/2;
				rectBmp.bottom  = rectBmp.top + ICON_PIXEL ;
				
				rectBmp.left += (rect.Width()-ICON_PIXEL)/2;
				rectBmp.right = rectBmp.left + ICON_PIXEL ;
			}
			//贴小图标
		...
		//	dc.BitBlt(rectBmp.left, rectBmp.top, rectBmp.Width (), rectBmp.Height(), &memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
			TransparentBlt2(dc.m_hDC, \
				rectBmp.left, rectBmp.top, ICON_PIXEL, ICON_PIXEL,\
				memDC.m_hDC, 0, 0, ICON_PIXEL, ICON_PIXEL, RGB(0xFF, 0, 0xFF));
			memDC.SelectObject (oldbitmap);
			cb.DeleteObject();
...
void CCEIconSoftKey::setToggleKeyIcon()
{
...
#define	WRITE_INPUT_TABLE	"/HIVE/RES_DAT/jHWRDic.dat"  //手写库
#define	WRITE_LEGEND_TABLE	"/HIVE/RES_DAT/LegendModel.dat"  //联想库

//#define	WRITE_INPUT_TABLE	"/HIVE/Res_DAT/write.dic"
...
int HWRE_LoadDict(unsigned char** hDic)
{
	...
	BOOL ret = HZInitCharacterRecognition(pDic);  //wangzhenxing20101202
	*hDic = pDic;
	
	return 0;
}

/*int HWRE_LoadLegend(unsigned long &dwHandle)
{
	FILE *in;
	long nLen;
	signed char* pDic;
	
	in = fopen(WRITE_LEGEND_TABLE, "rb");
	if( in == NULL )
		return -1;
	...
	long nBufSize = 0;
	g_pInitRes.pszTRLib = (const signed char *)pDic;
    jtAWErrCode Result = jtAW_GetExtBufSize(&g_pInitRes, &nBufSize);

	if(Result == jtAW_ERR_NONE)
	{
		g_pInitRes.pExtBuf = malloc(nBufSize);
		Result = jtAW_Init(&g_pInitRes, &dwHandle);
		if(Result != jtAW_ERR_NONE)
		{
			if(g_pInitRes.pExtBuf != NULL)
			{
				free(g_pInitRes.pExtBuf);
			}
			return -3;  //初始化失败
		}
	}
	...
	{
		if(g_pInitRes.pszTRLib != NULL)
		{
			free((void*)g_pInitRes.pszTRLib);
			pDic = NULL;
		}
		return -3;
	}
...
void HWRE_FreeLegend(unsigned long dwHandle)
{
	if(dwHandle > 0)
	{
		jtAW_Uninit(dwHandle);
	}
