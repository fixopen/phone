...
void CInputDlg::CreateCtrl()
{
	...
	//拼音软键盘
	//英文键盘
	int x1Local[] = {1, 31, 61, 91, 121, 151, 181, 211, 241, 269, 299, 329, 359, 389, 419, 449, 479};
	int y1Local[] = {3, 30, 57, 84, 110};
	CRect r2[64];
	...
	if( !(m_oSoftkeyP.Create(IDB_VOIPINPUTPINYIN, thePoint, 62, r2, vSoftKeyP, INPUTSOFTKEYRECT, this, 0, VOIPINPUT_PINYIN_IDC)) )
        return ;
	...
	VOIPINPUT_DELETE_VALUE, VOIPINPUT_COPY_VALUE, VOIPINPUT_PASTE_VALUE, VOIPINPUT_SPACE_VALUE, VOIPINPUT_ENTER_VALUE, VOIPINPUT_MOVE_VALUE, VOIPINPUT_HIDE_VALUE};
	if( !(m_oSoftkeyEng.Create(IDB_VOIPINPUTENG, thePoint, 48+7, r2, vSoftKeyD, INPUTSOFTKEYRECT, this, 0, VOIPINPUT_ENG_IDC)) )
        return ;
	m_oSoftkeyEng.ModifyStyle(0, SS_NOTIFY);
	if( !(m_oSoftkeySymbol.Create(IDB_VOIPINPUTFUHAO, thePoint, 48+7, r2, vSoftKeyF, INPUTSOFTKEYRECT, this, 0, VOIPINPUT_FUHAO_IDC)) )
        return ;
	...
	if( !(m_oSoftkeyHand.Create(IDB_VOIPINPUTH, thePoint, 37, r2, vSoftKeyH, INPUTSOFTKEYRECT, this, 0, VOIPINPUT_HAND_IDC)) )
        return ;
	m_oSoftkeyHand.SetHandWriteRect(CRect(221, 32, 336, 109), CRect(350, 32, 467, 109));
...
BOOL CInputDlg::MoveTo(int y)
{
	...
	MoveWindow(0, y, myRect.Width(), myRect.Height());
...
//	int style;
	...
	if(m_mInputMath == INPUTMATH_ENG)
	{
		if(m_iUpOrLowCase == INPUTMATH_D)
			m_oSoftkeyEng.SetParameters(IDB_VOIPINPUTENG1);
		else if(m_iUpOrLowCase == INPUTMATH_X)
			m_oSoftkeyEng.SetParameters(IDB_VOIPINPUTENG);
		m_oSoftkeyEng.ShowWindow(SW_SHOW);
	}
	else if(m_mInputMath == INPUTMATH_F)
	{
		m_oSoftkeySymbol.SetParameters(IDB_VOIPINPUTFUHAO);
		m_oSoftkeySymbol.ShowWindow(SW_SHOW);
	}
	else if(m_mInputMath == INPUTMATH_P)
	{
		m_oSoftkeyP.SetParameters(IDB_VOIPINPUTPINYIN);
		m_oSoftkeyP.ShowWindow(SW_SHOW);
	}
	else if(m_mInputMath == INPUTMATH_H)
	{
		m_oSoftkeyHand.SetParameters(IDB_VOIPINPUTH);
...
//	CPaintDC dc(this); // device context for painting
	...
	//	CRect r = CRect(m_oSoftkeyP.m_pKeys[0].sKeyRect.right+1, m_oSoftkeyP.m_pKeys[0].sKeyRect.top+1, m_oSoftkeyP.m_pKeys[1].sKeyRect.left-1, m_oSoftkeyP.m_pKeys[1].sKeyRect.bottom-1);
		...
		PutIconPartial(pDC, r1.left, r1.top, IDB_VOIPINPUTPINYIN, r1);
		for(int i=0;i<PY_HZ_SHOWLEN;i++)
		{
			...
			memset(wch, 0, sizeof(wch));
			mbstowcs( wch, txt, strlen(txt) );
			pDC->DrawText(wch, wcslen(wch), rA[i], DT_INTERNAL);
		}
		...
		PutIconPartial(pDC, r.left, r.top, IDB_VOIPINPUTPINYIN, r);
		if(strlen(m_chPYInPut))
		{
			...
			pDC->DrawText(wch, wcslen(wch), r, DT_INTERNAL);
		}
		ReleaseDC(pDC);
	}
	...
	else if(m_mInputMath == INPUTMATH_H)
	{
		...
		PutIconPartial(pDC, r.left,r.top, IDB_VOIPINPUTENG, r);
		...
		for(int i = 0; i < SX_HZ_SHOWLEN; i++)
		{
			...
			memset(wch, 0, sizeof(wch));
			mbstowcs( wch, txt, strlen(txt) );
			pDC->DrawText(wch, rA[i], DT_INTERNAL);	
		}
		ReleaseDC(pDC);
	}
	if(m_pOwnerEdit)
