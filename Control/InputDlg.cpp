...
#define	PINYIN_INPUT_TABLE	"\\HIVE\\RES_DAT\\pinyin.dat"
#define HZ_FIRST_CHAR_FILE	"\\HIVE\\RES_DAT\\firstchar.txt"
...
void CInputDlg::CreateCtrl()
{
	...
	//拼音软键盘
	//英文键盘
//	int x1Local[] = {8, 37, 66, 95, 124, 153, 182, 211, 240, 269, 298, 327, 356, 385, 414, 443, 472};
//	int y1Local[] = {2, 22, 48, 76, 105};
	int x1Local[] = {0, 30, 60, 90, 121, 150, 180, 210, 240, 270, 300, 329, 360, 390, 419, 449, 479};
	int y1Local[] = {0, 20, 50, 80, 104};
	CRect r2[64];
	...
	for(int i = 0; i < 62; i++)
	{
	    if(i < 14)
		{
			r2[i] = CRect(x1Local[i+2]+1, (y1Local[0]+1)*VHEIGHT, x1Local[i+2+1]-1, (y1Local[1]-1)*VHEIGHT);
		}
		else if(i < 30)
		{
			r2[i] = CRect(x1Local[i-14]+1, (y1Local[1]+1)*VHEIGHT, x1Local[i-14+1]-1, (y1Local[2]-1)*VHEIGHT);
		}
		else if(i < 46)
		{
			r2[i] = CRect(x1Local[i-30]+1, (y1Local[2]+1)*VHEIGHT, x1Local[i-30+1]-1, (y1Local[3]-1)*VHEIGHT);
		}
		else
		{
			r2[i] = CRect(x1Local[i-46]+1, (y1Local[3]+1)*VHEIGHT, x1Local[i-46+1]-1,(y1Local[4]-1)*VHEIGHT);
		}
	}
	...
	for(i = 0; i < 48; i++)
	{
		if(i < 16)
		{
			r2[i] = CRect(x1Local[i]+1+m_xPoint, (y1Local[1]+1+m_yPoint)*VHEIGHT, x1Local[i+1]-1+m_xPoint, (y1Local[2]-1+m_yPoint)*VHEIGHT);
		}
		else if(i < 32)
		{
			r2[i] = CRect(x1Local[i-16]+1+m_xPoint, (y1Local[2]+1+m_yPoint)*VHEIGHT, x1Local[i-16+1]-1+m_xPoint, (y1Local[3]-1+m_yPoint)*VHEIGHT);
		}
	
		else
		{
			r2[i] = CRect(x1Local[i-32]+1+m_xPoint, (y1Local[3]+1+m_yPoint)*VHEIGHT, x1Local[i-32+1]-1+m_xPoint, (y1Local[4]-1+m_yPoint)*VHEIGHT);
		}
	}

	for( i = 0; i < 7; i++)
	{
		r2[i+48] = CRect(x1Local[9+i]+1+m_xPoint, (y1Local[0]+1+m_yPoint)*VHEIGHT, x1Local[9+i+1]-1+m_xPoint, (y1Local[1]-1+m_yPoint)*VHEIGHT);
	}
	...
	for(i = 0; i < 37; i++)
	{
		if(i <= 15)
		{
			r2[i] = CRect(x1Local[i]+1, (y1Local[0]+1)*VHEIGHT, x1Local[i+1]-1, (y1Local[1]-1)*VHEIGHT);
		}
		else if(i <= 22)
		{
			r2[i] = CRect(x1Local[i-16]+1, (y1Local[1]+1)*VHEIGHT, x1Local[i-16+1]-1, (y1Local[2]-1)*VHEIGHT);
		}
		else if(i <= 29)
		{
			r2[i] = CRect(x1Local[i-23]+1, (y1Local[2]+1)*VHEIGHT, x1Local[i-23+1]-1, (y1Local[3]-1)*VHEIGHT);
		}
		else 
		{
			r2[i] = CRect(x1Local[i-30]+1, (y1Local[3]+1)*VHEIGHT, x1Local[i-30+1]-1, (y1Local[4]-1)*VHEIGHT);
		}
	}
	if( !(m_oSoftkeyHand.Create(Data::g_inputWriteBMPID[Data::g_skinstyle], thePoint, 37, r2, vSoftKeyH, INPUTSOFTKEYRECT, this, 0, VOIPINPUT_HAND_IDC)) )
        return ;
	m_oSoftkeyHand.SetHandWriteRect(CRect(217, 24*VHEIGHT, 343, 103*VHEIGHT), CRect(353, 24*VHEIGHT, 478, 103*VHEIGHT));
	m_oSoftkeyHand.ModifyStyle(0, SS_NOTIFY);
	
	m_legendDlg.Create(WS_CHILD|WS_VISIBLE, CRect(), this, IDD_DIALOG_LEGEND);  //wangzhenxing20101203
...
{
	m_legendDlg.ShowWindow(SW_HIDE);  //wangzhenxing20101203
...
void CInputDlg::ChangMode(int iMode,int iRangeOrUpLow)
{
	...
	{	
		...
		theApp.handWriteDlg_->SetOwnerEdit(m_pOwnerEdit);
