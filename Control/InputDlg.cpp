...
#define PINYIN_INPUT_TABLE  "/NandFlash/resource/res_dat/pinyin.dat"
#define HZ_FIRST_CHAR_FILE  "/NandFlash/resource/res_dat/firstchar.txt"
...
void InitPinYin() {
    ...
    FILE* fp = 0;
    fopen_s(&fp, PINYIN_INPUT_TABLE, "rb");
...
void InitHzFirstChar() {
    ...
    FILE* fp = 0;
    fopen_s(&fp, HZ_FIRST_CHAR_FILE, "rb");
...
char* hwstrncpy(char* strDest, const char* strSource, size_t count) {
    strncpy_s(strDest, 7, strSource, count);
    return strDest;
}
...
BOOL CInputDlg::OnInitDialog() {
    ...
    VERIFY(m_font.CreateFont(28,                        // nHeight
        0,                         // nWidth
        0,                         // nEscapement
        0,                         // nOrientation
        FW_NORMAL,                 // nWeight
...
void CInputDlg::CreateCtrl() {
    ...
    int x1Local[] = {
        7, 56, 105, 154, 203, 252, 301, 350, 399, 448, 497, 546, 595, 644, 693, 742, 791
    };
    int y1Local[] = {
        7, 57, 114, 171, 220
    };
    ...
    thePoint.y = m_yPoint;
    UINT16 vSoftKeyP[] = {
        HOUXUAN_LOCAL, HOUXUAN_LOCAL + 1, HOUXUAN_LOCAL + 2, HOUXUAN_LOCAL + 3, HOUXUAN_LOCAL + 4, ARROW_LEFT, ARROW_RIGHT, VOIPINPUT_DELETE_VALUE, VOIPINPUT_COPY_VALUE, VOIPINPUT_PASTE_VALUE, VOIPINPUT_SPACE_VALUE, VOIPINPUT_ENTER_VALUE, VOIPINPUT_MOVE_VALUE, VOIPINPUT_HIDE_VALUE,\
    ...
    UINT16 vSoftKeyD[] = {
        '-', '_', '@', '0', '1', '2', '3', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', '!', '?', ',', '.', '4', '5', '6', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', VOIPINPUT_WRITE_VALUE, VOIPINPUT_PINYIN_VALUE, VOIPINPUT_ENG_VALUE, VOIPINPUT_ZIFU_VALUE, '7', '8', '9', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', VOIPINPUT_COPSLOCK_VALUE, VOIPINPUT_DELETE_VALUE, VOIPINPUT_COPY_VALUE, VOIPINPUT_PASTE_VALUE, VOIPINPUT_SPACE_VALUE, VOIPINPUT_ENTER_VALUE, VOIPINPUT_MOVE_VALUE, VOIPINPUT_HIDE_VALUE
    };
    UINT16 vSoftKeyF[] = {
        '-', '_', '@', '#', '+', '-', '*', '/', '=', '%', '&', '~', '^', /*'￥'*/0xA3A4, '$', 0xA1EA, '!', '?', ',', '.', 0xA1A3, 0xA1A2, '<', '>', '(', ')', '[', ']', '{', '}', 0xA1B6, 0xA1B7, VOIPINPUT_WRITE_VALUE, VOIPINPUT_PINYIN_VALUE, VOIPINPUT_ENG_VALUE, VOIPINPUT_ZIFU_VALUE, ':', ';', 0xA1AD, 0xA6B5, 0xA6C1, 0xA6C2, 0xA6C3, '|', '\'', '\'', '"', '"', VOIPINPUT_DELETE_VALUE, VOIPINPUT_COPY_VALUE, VOIPINPUT_PASTE_VALUE, VOIPINPUT_SPACE_VALUE, VOIPINPUT_ENTER_VALUE, VOIPINPUT_MOVE_VALUE, VOIPINPUT_HIDE_VALUE
    };
    ...
    UINT16 vSoftKeyH[] = {
        HAND_REG1_VALUE, HAND_REG2_VALUE, HAND_REG3_VALUE, HAND_REG4_VALUE, HAND_REG5_VALUE, HAND_REG6_VALUE, HAND_REG7_VALUE, HAND_REG8_VALUE, HAND_REG9_VALUE, VOIPINPUT_DELETE_VALUE, VOIPINPUT_COPY_VALUE, VOIPINPUT_PASTE_VALUE, VOIPINPUT_SPACE_VALUE, VOIPINPUT_ENTER_VALUE, VOIPINPUT_MOVE_VALUE, VOIPINPUT_HIDE_VALUE,\
        '-', '_', '@', '0', '1', '2', '3', '!', '?', ',', '.', '4', '5', '6', VOIPINPUT_WRITE_VALUE, VOIPINPUT_PINYIN_VALUE, VOIPINPUT_ENG_VALUE, VOIPINPUT_ZIFU_VALUE, '7', '8', '9'
    }; //40
    for (int i = 0; i < 37; i++) {
        if (i <= 15) {
...
void CInputDlg::OnSoftKeyP(NMHDR_* hm, LRESULT*) {
        ...
        || wParam == '?' || wParam == ',' || wParam == '.' || (wParam >= '0' && wParam <= '9') || wParam == VOIPINPUT_SPACE_VALUE || wParam == VOIPINPUT_ENTER_VALUE)   //立即可以显示的字符
    {
        ...
        if (m_pOwnerEdit) {
            ...
            mbstowcs_s(&convertCount, wch, 2, txt, strlen(txt));
...
void CInputDlg::OnPaint() {
    ...
    if (m_mInputMath == INPUTMATH_P) {
        ...
        CRect rA[] = {
            m_oSoftkeyP.m_pKeys[0].sKeyRect, m_oSoftkeyP.m_pKeys[1].sKeyRect, m_oSoftkeyP.m_pKeys[2].sKeyRect, m_oSoftkeyP.m_pKeys[3].sKeyRect, m_oSoftkeyP.m_pKeys[4].sKeyRect, m_oSoftkeyP.m_pKeys[5].sKeyRect
        };
        ...
        if (strlen(m_chPYInPut)) {
            ...
            memset(wch, 0, sizeof(wch));
            size_t convertCount = 0;
            mbstowcs_s(&convertCount, wch, 128, m_chPYInPut, strlen(m_chPYInPut));
            pDC->SetBkMode(TRANSPARENT);
        ...
        for (int i = 0; i < SX_HZ_SHOWLEN; i++) {
            ...
            memset(wch, 0, sizeof(wch));
            size_t convertCount = 0;
            mbstowcs_s(&convertCount, wch, 128, txt, strlen(txt));
            pDC->SetBkMode(TRANSPARENT);
