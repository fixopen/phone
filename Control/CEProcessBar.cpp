...
void CCEProcessBar::OnDrawBar_HDC()
{
	if(!IsWindowVisible())
		return;
	...
	if(m_nStyle <= 1)
	{
		...
		//	pdc->FillRect(&rt, &bBr);
			...
			::FillRect(pdc, &lRect, (HBRUSH)lBr.m_hObject);
			::FillRect(pdc, &rRect, (HBRUSH)rBr.m_hObject);
			...
			if(m_BitmapID > 0)
			{
				HBITMAP cb = ::LoadBitmap(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(m_BitmapID));//载入资源
				HDC memDC = ::CreateCompatibleDC( pdc);//创建内存设备上下文
				HBITMAP oldbitmap=(HBITMAP)::SelectObject (memDC, cb);//将位图选入当前设备场景
				CRect rect = rt;
				rect.left = lRect.right - MIDDLEBMP_SIZE;
				rect.right = lRect.right + MIDDLEBMP_SIZE;
				::BitBlt (pdc, rect.left, rect.top, rect.Width (), rect.Height(), memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
		...
		else
		{
			...
			int nMidWidth = 0;
			if(m_BitmapID > 0)
					nMidWidth = MIDDLEBMP_SIZE;
			
			if(m_BitmapIDBottom > 0)
			{
				
				HBITMAP cb = ::LoadBitmap(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(m_BitmapIDBottom));//载入资源
				HDC memDC = ::CreateCompatibleDC( pdc);//创建内存设备上下文
				HBITMAP oldbitmap=(HBITMAP)::SelectObject (memDC, cb);//将位图选入当前设备场景

				CRect rect = rt;
				//pdc->BitBlt (rect.left, rect.top, rect.Width (), rect.Height(), &memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
				::BitBlt (pdc, lRect.right+nMidWidth, lRect.top, rect.Width()-lRect.right, lRect.Height(), memDC, lRect.right+nMidWidth, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
				//TransparentBlt2(pdc->m_hDC, rect.left, rect.top, rect.Width (), rect.Height(), memDC.m_hDC, 0, 0, rect.Width (), rect.Height(), RGB(0xFF, 0, 0xFF));
				::SelectObject (memDC, oldbitmap);
				::DeleteObject(cb);
				::DeleteDC(memDC);
			...
			if(m_BitmapIDTop > 0)
			{
				HBITMAP cb = ::LoadBitmap(AfxGetApp()->m_hInstance, MAKEINTRESOURCE(m_BitmapIDTop));//载入资源
				HDC memDC = ::CreateCompatibleDC( pdc);//创建内存设备上下文
				HBITMAP oldbitmap=(HBITMAP)::SelectObject (memDC, cb);//将位图选入当前设备场景

				CRect rect = rt;

				if(lRect.Width()-nMidWidth > 0)
					::BitBlt (pdc, rect.left, rect.top, lRect.Width()-nMidWidth, lRect.Height(), memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
				//TransparentBlt2(pdc->m_hDC, rect.left, rect.top, lRect.Width(), lRect.Height(), memDC.m_hDC, 0, 0, lRect.Width(), lRect.Height(), RGB(0xFF, 0, 0xFF));
				::SelectObject (memDC, oldbitmap);
				::DeleteObject(cb);
				::DeleteDC(memDC);
			...
			if(m_BitmapID > 0)
			{
				...
				CRect rect = rt;
				rect.left = lRect.right - nMidWidth;
				rect.right = lRect.right + nMidWidth;
				if(rect.left < 0)
				{
					rect.left = 0;
					rect.right = nMidWidth*2;
				}
				else if(rect.right > rt.right)
				{
					rect.left = rt.right - nMidWidth*2;
					rect.right = rt.right;
				}
				::BitBlt (pdc, rect.left, rect.top, rect.Width (), rect.Height(), memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
				::SelectObject (memDC, oldbitmap);
	...
	else      //竖条
	{
		...
		{
			CBrush bBr = Data::g_progressBackRGB[Data::g_skinstyle]; 
			::FillRect(pdc, &rt, (HBRUSH)bBr.m_hObject);
		...
		//	bRect.bottom -= MIDDLEBMP_SIZE;
			...
			CBrush rBr = Data::g_progressRightRGB[Data::g_skinstyle]; 
			CBrush b_Br = Data::g_progressLeftRGB[Data::g_skinstyle]; 
			::FillRect(pdc, &tRect, (HBRUSH)rBr.m_hObject);
			::FillRect(pdc, &bRect, (HBRUSH)b_Br.m_hObject);
...
//			pdc->FillSolidRect(rt, m_backRGB);
			...
			if(m_BitmapIDBottom > 0)
			{
				...
				CRect rect = rt;
				::BitBlt (pdc, tRect.left, tRect.top, tRect.Width (), tRect.Height(), memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
				//TransparentBlt2(pdc->m_hDC, tRect.left, tRect.top, tRect.Width (), tRect.Height(), memDC.m_hDC, 0, 0, tRect.Width (), tRect.Height(), RGB(0xFF, 0, 0xFF));
				::SelectObject (memDC, oldbitmap);
				::DeleteObject(cb);
				::DeleteDC(memDC);
			...
			if(m_BitmapIDTop > 0)
			{
				...
				CRect rect = rt;
				::BitBlt (pdc, bRect.left, bRect.top, bRect.Width(), bRect.Height(), memDC, rt.Width()-bRect.Width(), rt.Height()-bRect.Height(), SRCCOPY);//将内存场景的图象拷贝到屏幕上
				//TransparentBlt2(pdc->m_hDC, bRect.left, bRect.top, bRect.Width(), bRect.Height(), memDC.m_hDC, rt.Width()-bRect.Width(), rt.Height()-bRect.Height(), bRect.Width(), bRect.Height(), RGB(0xFF, 0, 0xFF));
				::SelectObject (memDC, oldbitmap);
				::DeleteObject(cb);
				::DeleteDC(memDC);
