	...
	if(m_nStyle <= 1)
	{
		...
		else
		{
			...
			if(m_BitmapIDBottom > 0)
			{
				...
				::BitBlt (pdc, lRect.right+MIDDLEBMP_SIZE, lRect.top, rect.Width()-lRect.right, lRect.Height(), memDC, lRect.right+MIDDLEBMP_SIZE, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
			...
			if(m_BitmapIDTop > 0)
			{
				...
				if(lRect.Width()-MIDDLEBMP_SIZE > 0)
					::BitBlt (pdc, rect.left, rect.top, lRect.Width()-MIDDLEBMP_SIZE, lRect.Height(), memDC, 0, 0, SRCCOPY);//将内存场景的图象拷贝到屏幕上
