...
void CCELunarder::SetData(UINT16 year, UINT8 month, UINT8 day, BOOL reDraw) {
    ...
    if (reDraw) {
        ...
        sprintf_s(txt2, 24, "%d %s %s", m_nY, txt1, Data::LanguageResource::Get(Data::RI_COMN_YEARSTC).c_str());
        CString s = txt2;
        if (m_pStcYear) {
            m_pStcYear->SetWindowText(s);
        }
        sprintf_s(txt, 24, "%d%s", m_nM, Data::LanguageResource::Get(Data::RI_COMN_MONTHSTC).c_str());
        s = txt;
        if (m_pStcMonth) {
...
LRESULT CCELunarder::OnDaySelect(WPARAM w, LPARAM l) {
    ...
    int n = wcstombs_s(&convertCount, txt, 3, (LPTSTR)(LPCTSTR)s, 3);
    txt[n] = '\0';
