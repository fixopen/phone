...
public:
	...
	DWORD SetIcon(int nIconIn, CSize sizeIn = CSize(16,16), int nIconOut = NULL, CSize sizeOut = CSize(16,16), int nIconDis = NULL, CSize sizeDis = CSize(16,16));
	DWORD SetIcon(HICON hIconIn, CSize sizeIn = CSize(16,16), HICON hIconOut = NULL, CSize sizeOut = CSize(16,16), HICON hIconDis = NULL, CSize sizeDis = CSize(16,16));
...
class CCEBmpButton: public CButton
{
public:
	int	m_nStatus;    // 0 up 1 down
	BOOL m_bToggle;
	BOOL m_bToggleValue;
	int m_nBitmapUp;
	int m_nBitmapDown;
	...
	BOOL Create(LPCTSTR lpszCaption, int nIDUp, int nIDDown, DWORD dwStyle,	const RECT& rect, CWnd* pParentWnd, UINT nID, int fontSize = 16);
	
	void SetToggleValue(BOOL flag){m_bToggleValue = flag;Invalidate();}
	BOOL GetToggle(){return m_bToggleValue;}
	void SetIsToggle(BOOL flag){m_bToggle = flag;}
	void SetTxtColor(COLORREF cl){m_txtRGB = cl;}
	void SetBmpID(int nIDUp, int nIDDown, BOOL redraw = TRUE);
	void SetBmpOffSet(int x, int y){m_nBmpOffy = y; m_nBmpOffx = x;}
...
private:
	...
	// Overrides
	// ClassWizard generated virtual function overrides
	//{{AFX_VIRTUAL(CCEBmpButton)
	//}}AFX_VIRTUAL
	
	// Implementation
public:
	virtual ~CCEBmpButton();
	
	// Generated message map functions
protected:
	virtual void PreSubclassWindow();
	//{{AFX_MSG(CCEBmpButton)
